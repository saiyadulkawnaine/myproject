...
var MsCompanyUserModel = __webpack_require__(9);

var MsCompanyUserController = function () {
	function MsCompanyUserController(MsCompanyUserModel) {
		_classCallCheck(this, MsCompanyUserController);

		this.MsCompanyUserModel = MsCompanyUserModel;
		this.formId = 'companyuserFrm';
		this.dataTable = '#companyuserTbl';
		this.route = msApp.baseUrl() + "/companyuser";
	}

	_createClass(MsCompanyUserController, [{
		key: 'submit',
		value: function submit() {
			var formObj = msApp.get('companyuserFrm');
			var i = 1;
			$.each($('#companyuserTbl').datagrid('getChecked'), function (idx, val) {
				formObj['company_id[' + i + ']'] = val.id;

				i++;
			});
			this.MsCompanyUserModel.save(this.route, 'POST', msApp.qs.stringify(formObj), this.response);
		}
	}, {
		key: 'create',
		value: function create() {
			var user_id = $('#buyerFrm  [name=id]').val();
			var data = axios.get(msApp.baseUrl() + "/companyuser/create?_id=" + user_id);
			data.then(function (response) {
				$('#companyuserTbl').datagrid({
					checkbox: true,
					rownumbers: true,
					data: response.data.unsaved,

					columns: [[{ field: 'ck', checkbox: true, width: 40 }, { field: 'name', title: 'Company', width: 100 }]]
				});

				$('#companyusersavedTbl').datagrid({
					rownumbers: true,
					data: response.data.saved,
					columns: [[{ field: 'name', title: 'Company', width: 100 }, { field: 'action', title: '', width: 60, formatter: MsCompanyUser.formatDetail }]]
				});
			}).catch(function (error) {
				console.log(error);
			});
		...
		value: function remove() {
			...
			this.MsCompanyUserModel.save(this.route + "/" + formObj.id, 'DELETE', null, this.response);
		}
	}, {
		key: 'delete',
		value: function _delete(event, id) {
			...
			alert(id);
			this.MsCompanyUserModel.save(this.route + "/" + id, 'DELETE', null, this.response);
		}
	}, {
		key: 'response',
		...
		value: function edit(index, row) {
			...
			this.MsCompanyUserModel.get(index, row);
		}
	}, {
		key: 'showGrid',
		...
		value: function formatDetail(value, row) {
			return '<a href="javascript:void(0)"  onClick="MsCompanyUser.delete(event,' + row.company_user_id + ')"><span class="btn btn-danger btn-xs"><i class="fa fa-search"></i>Delete</span></a>';
		}
	}]);

	return MsCompanyUserController;
}();

window.MsCompanyUser = new MsCompanyUserController(new MsCompanyUserModel());
...
var MsCompanyUserModel = function (_MsModel) {
	_inherits(MsCompanyUserModel, _MsModel);

	function MsCompanyUserModel() {
		_classCallCheck(this, MsCompanyUserModel);

		return _possibleConstructorReturn(this, (MsCompanyUserModel.__proto__ || Object.getPrototypeOf(MsCompanyUserModel)).call(this));
	}

	return MsCompanyUserModel;
}(MsModel);

module.exports = MsCompanyUserModel;
