...
var MsNegotiationReportModel = __webpack_require__(191);
__webpack_require__(2);

var MsNegotiationReportController = function () {
	function MsNegotiationReportController(MsNegotiationReportModel) {
		_classCallCheck(this, MsNegotiationReportController);

		this.MsNegotiationReportModel = MsNegotiationReportModel;
		this.formId = 'negotiationreportFrm';
		this.dataTable = '#negotiationreportTbl';
		this.route = msApp.baseUrl() + "/negotiationreport";
	}

	_createClass(MsNegotiationReportController, [{
		key: 'getParams',
		value: function getParams() {
			var params = {};
			params.date_from = $('#negotiationreportFrm  [name=date_from]').val();
			params.date_to = $('#negotiationreportFrm  [name=date_to]').val();
			params.bank_id = $('#negotiationreportFrm  [name=bank_id]').val();
			params.beneficiary_id = $('#negotiationreportFrm  [name=beneficiary_id]').val();
			params.buyer_id = $('#negotiationreportFrm  [name=buyer_id]').val();
			params.file_no = $('#negotiationreportFrm  [name=file_no]').val();
			params.submission_id = $('#negotiationreportFrm  [name=submission_id]').val();
			params.possible_date_from = $('#negotiationreportFrm  [name=possible_date_from]').val();
			params.possible_date_to = $('#negotiationreportFrm  [name=possible_date_to]').val();
			return params;
		}
	}, {
		key: 'get',
		value: function get() {
			...
			var d = axios.get(this.route + '/htmlgrid', { params: params }).then(function (response) {
				$('#negotiationreportTbl').datagrid('loadData', response.data);
			}).catch(function (error) {
				console.log(error);
			});
		}
		...
		value: function formatDetail(value, row) {
			return '<a href="javascript:void(0)"  onClick="MsNegotiationReport.latter(' + row.id + ')"><span class="btn btn-danger btn-xs"><i class="fa fa-search"></i>Nego</span></a>';
		}
	}, {
		key: 'getbuyersummery',
		value: function getbuyersummery() {
			var params = this.getParams();
			var d = axios.get(this.route + '/buyersummery', { params: params }).then(function (response) {
				$('#negotiationreportbuyerTbl').datagrid('loadData', response.data);
			}).catch(function (error) {
				console.log(error);
			});
		}
		...
		key: 'showGridBuyer',
		value: function showGridBuyer(data) {
			var dg = $('#negotiationreportbuyerTbl');
			dg.datagrid({
				border: false,
				singleSelect: true,
				showFooter: true,
		...
		key: 'getbuyerfollowup',
		value: function getbuyerfollowup() {
			var params = this.getParams();
			var d = axios.get(this.route + '/buyerfollowupdata', { params: params }).then(function (response) {
				$('#buyerfollowupWindow').window('open');
				$('#buyerfollowupTbl').datagrid('loadData', response.data);
			}).catch(function (error) {
				console.log(error);
			});
		}
	}, {
		key: 'showGridBuyerFollowUp',
		value: function showGridBuyerFollowUp(data) {
			var dfg = $('#buyerfollowupTbl');
			dfg.datagrid({
				border: false,
				singleSelect: true,
				showFooter: true,
			...
			dfg.datagrid('enableFilter').datagrid('loadData', data);
		}
	}]);

	return MsNegotiationReportController;
}();

window.MsNegotiationReport = new MsNegotiationReportController(new MsNegotiationReportModel());
MsNegotiationReport.showGrid([]);
MsNegotiationReport.showGridBuyer([]);
MsNegotiationReport.showGridBuyerFollowUp([]);
...
var MsNegotiationReportModel = function (_MsModel) {
	_inherits(MsNegotiationReportModel, _MsModel);

	function MsNegotiationReportModel() {
		_classCallCheck(this, MsNegotiationReportModel);

		return _possibleConstructorReturn(this, (MsNegotiationReportModel.__proto__ || Object.getPrototypeOf(MsNegotiationReportModel)).call(this));
	}

	return MsNegotiationReportModel;
}(MsModel);

module.exports = MsNegotiationReportModel;
