...
var MsCapacityDistributionBuyerTeamModel = __webpack_require__(265);

var MsCapacityDistributionBuyerTeamController = function () {
	function MsCapacityDistributionBuyerTeamController(MsCapacityDistributionBuyerTeamModel) {
		_classCallCheck(this, MsCapacityDistributionBuyerTeamController);

		this.MsCapacityDistributionBuyerTeamModel = MsCapacityDistributionBuyerTeamModel;
		this.formId = 'capacitydistributionbuyerteamFrm';
		this.dataTable = '#capacitydistributionbuyerteamTbl';
		this.route = msApp.baseUrl() + "/capacitydistributionbuyerteam";
	}

	_createClass(MsCapacityDistributionBuyerTeamController, [{
		key: 'submit',
		value: function submit() {
			var formObj = msApp.get(this.formId);
			if (formObj.id) {
				this.MsCapacityDistributionBuyerTeamModel.save(this.route + "/" + formObj.id, 'PUT', msApp.qs.stringify(formObj), this.response);
			} else {
				this.MsCapacityDistributionBuyerTeamModel.save(this.route, 'POST', msApp.qs.stringify(formObj), this.response);
			}
		}
	}, {
		key: 'remove',
		value: function remove() {
			var formObj = msApp.get(this.formId);
			this.MsCapacityDistributionBuyerTeamModel.save(this.route + "/" + formObj.id, 'DELETE', null, this.response);
		}
	}, {
		key: 'delete',
		value: function _delete(event, id) {
			event.stopPropagation();
			this.MsCapacityDistributionBuyerTeamModel.save(this.route + "/" + id, 'DELETE', null, this.response);
		}
	}, {
		key: 'response',
		value: function response(d) {
			$('#capacitydistributionbuyerteamTbl').datagrid('reload');
			//$('#CapacityDistributionBuyerTeamFrm  [name=id]').val(d.id);
			msApp.resetForm('capacitydistributionbuyerteamFrm');
		}
	}, {
		key: 'edit',
		value: function edit(index, row) {
			...
			this.MsCapacityDistributionBuyerTeamModel.get(index, row);
		}
	}, {
		key: 'showGrid',
		...
		value: function formatDetail(value, row) {
			return '<a href="javascript:void(0)"  onClick="MsCapacityDistributionBuyerTeam.delete(event,' + row.id + ')"><span class="btn btn-danger btn-xs"><i class="fa fa-search"></i>Delete</span></a>';
		}
	}]);

	return MsCapacityDistributionBuyerTeamController;
}();

window.MsCapacityDistributionBuyerTeam = new MsCapacityDistributionBuyerTeamController(new MsCapacityDistributionBuyerTeamModel());
MsCapacityDistributionBuyerTeam.showGrid();
...
var MsCapacityDistributionBuyerTeamModel = function (_MsModel) {
	_inherits(MsCapacityDistributionBuyerTeamModel, _MsModel);

	function MsCapacityDistributionBuyerTeamModel() {
		_classCallCheck(this, MsCapacityDistributionBuyerTeamModel);

		return _possibleConstructorReturn(this, (MsCapacityDistributionBuyerTeamModel.__proto__ || Object.getPrototypeOf(MsCapacityDistributionBuyerTeamModel)).call(this));
	}

	return MsCapacityDistributionBuyerTeamModel;
}(MsModel);

module.exports = MsCapacityDistributionBuyerTeamModel;
