...
var MsJobModel = __webpack_require__(319);

var MsJobController = function () {
	function MsJobController(MsJobModel) {
		_classCallCheck(this, MsJobController);
...
$('#jobtabs').tabs({
	onSelect: function onSelect(title, index) {
		...
		var job_id = $('#jobFrm  [name=id]').val();
		var job_no = $('#jobFrm  [name=job_no]').val();
		var style_id = $('#jobFrm  [name=style_id]').val();

		var data = {};
		data.style_id = style_id;
		var stylegmts = msApp.getJson('stylegmts', data);
		if (index == 1) {
		...
		if (index == 3) {
			...
			var stylecolors = msApp.getJson('stylecolor', data);
			stylecolors.then(function (response) {
				$('#salesordercolorFrm [name="style_color_id"]').empty();
				$('#salesordercolorFrm [name="style_color_id"]').append('<option value="">-Select-</option>');
				$.each(response.data, function (key, value) {
					$('#salesordercolorFrm [name="style_color_id"]').append('<option value="' + value.id + '">' + value.name + '</option>');
				});
			}).catch(function (error) {
				console.log(error);
			});
			$('#salesordersizeFrm  [name=job_id]').val($('#jobFrm  [name=id]').val());
			$('#salesordersizeFrm  [name=sale_order_id]').val($('#salesorderFrm  [name=id]').val());
			$('#salesordersizeFrm  [name=sale_order_country_id]').val($('#salesordercountryFrm  [name=id]').val());
			$('#salesordersizeFrm  [name=sale_order_color_id]').val($('#salesordercolorFrm  [name=id]').val());
			/*data.sale_order_country_id=$('#salesordercolorFrm  [name=sale_order_country_id]').val()
   let stylesizes = msApp.getJson('salesordersize/create',data);
   stylesizes.then(function (response) {
   $('#sizetable').html(response.data);
   $('#salesordersizeFrm  [name=job_id]').val($('#jobFrm  [name=id]').val())
   $('#salesordersizeFrm  [name=sale_order_id]').val($('#salesorderFrm  [name=id]').val())
   $('#salesordersizeFrm  [name=sale_order_country_id]').val($('#salesordercountryFrm  [name=id]').val())
   $('#salesordersizeFrm  [name=sale_order_color_id]').val($('#salesordercolorFrm  [name=id]').val())
   ...
   console.log(error);
		...
		if (index == 4) {
			$('#salesordersizeFrm  [name=sale_order_country_id]').val($('#salesordercountryFrm  [name=id]').val());
			var _stylecolors = msApp.getJson('stylesize', data);
			_stylecolors.then(function (response) {
				$('#salesordersizeFrm [name="style_size_id"]').empty();
				$('#salesordersizeFrm [name="style_size_id"]').append('<option value="">-Select-</option>');
				$.each(response.data, function (key, value) {
					$('#salesordersizeFrm [name="style_size_id"]').append('<option value="' + value.id + '">' + value.name + '</option>');
				});
			}).catch(function (error) {
				console.log(error);
			});
...
var MsJobModel = function (_MsModel) {
	_inherits(MsJobModel, _MsModel);

	function MsJobModel() {
		_classCallCheck(this, MsJobModel);

		return _possibleConstructorReturn(this, (MsJobModel.__proto__ || Object.getPrototypeOf(MsJobModel)).call(this));
	}

	return MsJobModel;
}(MsModel);

module.exports = MsJobModel;
