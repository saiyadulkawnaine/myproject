...
var MsCapacityDistributionBuyerModel = __webpack_require__(262);

var MsCapacityDistributionBuyerController = function () {
	function MsCapacityDistributionBuyerController(MsCapacityDistributionBuyerModel) {
		_classCallCheck(this, MsCapacityDistributionBuyerController);

		this.MsCapacityDistributionBuyerModel = MsCapacityDistributionBuyerModel;
		this.formId = 'capacityDistributionBuyerFrm';
		this.dataTable = '#capacityDistributionBuyerTbl';
		this.route = msApp.baseUrl() + "/capacitydistributionbuyer";
	}

	_createClass(MsCapacityDistributionBuyerController, [{
		key: 'submit',
		value: function submit() {
			var formObj = msApp.get(this.formId);
			if (formObj.id) {
				this.MsCapacityDistributionBuyerModel.save(this.route + "/" + formObj.id, 'PUT', msApp.qs.stringify(formObj), this.response);
			} else {
				this.MsCapacityDistributionBuyerModel.save(this.route, 'POST', msApp.qs.stringify(formObj), this.response);
			}
		}
	}, {
		key: 'remove',
		value: function remove() {
			var formObj = msApp.get(this.formId);
			this.MsCapacityDistributionBuyerModel.save(this.route + "/" + formObj.id, 'DELETE', null, this.response);
		}
	}, {
		key: 'delete',
		value: function _delete(event, id) {
			event.stopPropagation();
			this.MsCapacityDistributionBuyerModel.save(this.route + "/" + id, 'DELETE', null, this.response);
		}
	}, {
		key: 'response',
		value: function response(d) {
			$('#capacityDistributionBuyerTbl').datagrid('reload');
			//$('#CapacityDistributionBuyerFrm  [name=id]').val(d.id);
			msApp.resetForm('capacityDistributionBuyerFrm');
		}
	}, {
		key: 'edit',
		value: function edit(index, row) {
			...
			this.MsCapacityDistributionBuyerModel.get(index, row);
		}
	}, {
		key: 'showGrid',
		...
		value: function formatDetail(value, row) {
			return '<a href="javascript:void(0)"  onClick="MsCapacityDistributionBuyer.delete(event,' + row.id + ')"><span class="btn btn-danger btn-xs"><i class="fa fa-search"></i>Delete</span></a>';
		}
	}]);

	return MsCapacityDistributionBuyerController;
}();

window.MsCapacityDistributionBuyer = new MsCapacityDistributionBuyerController(new MsCapacityDistributionBuyerModel());
MsCapacityDistributionBuyer.showGrid();
...
var MsCapacityDistributionBuyerModel = function (_MsModel) {
	_inherits(MsCapacityDistributionBuyerModel, _MsModel);

	function MsCapacityDistributionBuyerModel() {
		_classCallCheck(this, MsCapacityDistributionBuyerModel);

		return _possibleConstructorReturn(this, (MsCapacityDistributionBuyerModel.__proto__ || Object.getPrototypeOf(MsCapacityDistributionBuyerModel)).call(this));
	}

	return MsCapacityDistributionBuyerModel;
}(MsModel);

module.exports = MsCapacityDistributionBuyerModel;
