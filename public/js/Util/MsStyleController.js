...
var MsStyleModel = __webpack_require__(285);

var MsStyleController = function () {
	function MsStyleController(MsStyleModel) {
		_classCallCheck(this, MsStyleController);
	...
	_createClass(MsStyleController, [{
		key: 'submit',
		...
		value: function edit(index, row) {
			...
			this.MsStyleModel.get(index, row);
			msApp.resetForm('stylegmtsFrm');
		...
		key: 'getTeamMember',
		value: function getTeamMember(team_id) {
			var data = {};
			data.team_id = team_id;
			msApp.getJson('teammember', data).then(function (response) {
...
$('#styletabs').tabs({
	onSelect: function onSelect(title, index) {
		...
		var style_id = $('#styleFrm  [name=id]').val();
		var style_ref = $('#styleFrm  [name=style_ref]').val();
		...
		if (index == 3) {
			...
			stylegmts.then(function (response) {
				$('#stylecolorFrm [name="style_gmt_id"]').empty();
				$('#stylecolorFrm [name="style_gmt_id"]').append('<option value="">-Select-</option>');
				$.each(response.data, function (key, value) {
					$('#stylecolorFrm [name="style_gmt_id"]').append('<option value="' + value.id + '">' + value.name + '</option>');
				});
			}).catch(function (error) {
				console.log(error);
			});
		...
		if (index == 4) {
			...
			stylegmts.then(function (response) {
				$('#stylesizeFrm [name="style_gmt_id"]').empty();
				$('#stylesizeFrm [name="style_gmt_id"]').append('<option value="">-Select-</option>');
				$.each(response.data, function (key, value) {
					$('#stylesizeFrm [name="style_gmt_id"]').append('<option value="' + value.id + '">' + value.name + '</option>');
				});
			}).catch(function (error) {
				console.log(error);
			});
...
var MsStyleModel = function (_MsModel) {
	_inherits(MsStyleModel, _MsModel);

	function MsStyleModel() {
		_classCallCheck(this, MsStyleModel);

		return _possibleConstructorReturn(this, (MsStyleModel.__proto__ || Object.getPrototypeOf(MsStyleModel)).call(this));
	}

	return MsStyleModel;
}(MsModel);

module.exports = MsStyleModel;
