...
var MsBulkFabricPurchaseDtlsQtyModel = __webpack_require__(424);

var MsBulkFabricPurchaseDtlsQtyController = function () {
	function MsBulkFabricPurchaseDtlsQtyController(MsBulkFabricPurchaseDtlsQtyModel) {
		_classCallCheck(this, MsBulkFabricPurchaseDtlsQtyController);

		this.MsBulkFabricPurchaseDtlsQtyModel = MsBulkFabricPurchaseDtlsQtyModel;
		this.formId = 'bulkfabricpurchaseqtyFrm';
		this.dataTable = '#bulkfabricpurchasedtlsqtyTbl';
		this.route = msApp.baseUrl() + "/bulkfabricpurchasedtlsqty";
	}

	_createClass(MsBulkFabricPurchaseDtlsQtyController, [{
		key: 'submit',
		value: function submit() {
			var formObj = msApp.get(this.formId);
			if (formObj.id) {
				this.MsBulkFabricPurchaseDtlsQtyModel.save(this.route + "/" + formObj.id, 'PUT', msApp.qs.stringify(formObj), this.response);
			} else {
				this.MsBulkFabricPurchaseDtlsQtyModel.save(this.route, 'POST', msApp.qs.stringify(formObj), this.response);
			}
		}
	}, {
		key: 'resetForm',
		...
		value: function remove() {
			var formObj = msApp.get(this.formId);
			this.MsBulkFabricPurchaseDtlsQtyModel.save(this.route + "/" + formObj.id, 'DELETE', null, this.response);
		}
	}, {
		key: 'delete',
		value: function _delete(event, id) {
			event.stopPropagation();
			this.MsBulkFabricPurchaseDtlsQtyModel.save(this.route + "/" + id, 'DELETE', null, this.response);
		}
	}, {
		key: 'response',
		value: function response(d) {
			//$('#bulkfabricpurchaseTbl').datagrid('reload');
			MsBulkFabricDtlsPurchase.fabricSearchGrid({ rows: {} });
			var purchase_order_id = $('#bulkfabricpurchaseqtyFrm  [name=id]').val();
			MsBulkFabricDtlsPurchase.get(purchase_order_id);
		}
	}, {
		key: 'edit',
		value: function edit(index, row) {
			...
			this.MsBulkFabricPurchaseDtlsQtyModel.get(index, row);
		}
	}, {
		key: 'showGrid',
		value: function showGrid(data) {
			var dg = $('#bulkfabricpurchasedtlsqtyTbl');
			dg.datagrid({
				border: false,
				fit: true,
				singleSelect: false,
		...
		value: function formatDetail(value, row) {
			return '<a href="javascript:void(0)"  onClick="MsBulkFabricDtlsPurchaseQty.delete(event,' + row.id + ')"><span class="btn btn-danger btn-xs"><i class="fa fa-search"></i>Delete</span></a>';
		}
	}, {
		key: 'openQtyWindow',
		value: function openQtyWindow(id) {
			if (!id) {
				alert('Save First');
				return;
			}
			var data = axios.get(msApp.baseUrl() + "/bulkfabricpurchasedtlsqty/create?bulk_fabric_purchase_dtl_id=" + id);
			var g = data.then(function (response) {
			...
			g.then(function (response) {
				$('#bulkfabricpurchaseqtyfabricWindow').window('open');
			});
		}
	}]);

	return MsBulkFabricPurchaseDtlsQtyController;
}();

window.MsBulkFabricDtlsPurchaseQty = new MsBulkFabricPurchaseDtlsQtyController(new MsBulkFabricPurchaseDtlsQtyModel());
...
var MsBulkFabricPurchaseDtlsQtyModel = function (_MsModel) {
	_inherits(MsBulkFabricPurchaseDtlsQtyModel, _MsModel);

	function MsBulkFabricPurchaseDtlsQtyModel() {
		_classCallCheck(this, MsBulkFabricPurchaseDtlsQtyModel);

		return _possibleConstructorReturn(this, (MsBulkFabricPurchaseDtlsQtyModel.__proto__ || Object.getPrototypeOf(MsBulkFabricPurchaseDtlsQtyModel)).call(this));
	}

	return MsBulkFabricPurchaseDtlsQtyModel;
}(MsModel);

module.exports = MsBulkFabricPurchaseDtlsQtyModel;
