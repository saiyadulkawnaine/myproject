...
var MsCapacityDistributionModel = __webpack_require__(259);

var MsCapacityDistributionController = function () {
	function MsCapacityDistributionController(MsCapacityDistributionModel) {
		_classCallCheck(this, MsCapacityDistributionController);

		this.MsCapacityDistributionModel = MsCapacityDistributionModel;
		this.formId = 'capacitydistributionFrm';
		this.dataTable = '#capacitydistributionTbl';
		this.route = msApp.baseUrl() + "/capacitydistribution";
	}

	_createClass(MsCapacityDistributionController, [{
		key: 'submit',
		value: function submit() {
			var formObj = msApp.get(this.formId);
			if (formObj.id) {
				this.MsCapacityDistributionModel.save(this.route + "/" + formObj.id, 'PUT', msApp.qs.stringify(formObj), this.response);
			} else {
				this.MsCapacityDistributionModel.save(this.route, 'POST', msApp.qs.stringify(formObj), this.response);
			}
		}
	}, {
		key: 'remove',
		value: function remove() {
			var formObj = msApp.get(this.formId);
			this.MsCapacityDistributionModel.save(this.route + "/" + formObj.id, 'DELETE', null, this.response);
		}
	}, {
		key: 'delete',
		value: function _delete(event, id) {
			event.stopPropagation();
			this.MsCapacityDistributionModel.save(this.route + "/" + id, 'DELETE', null, this.response);
		}
	}, {
		key: 'response',
		value: function response(d) {
			$('#capacitydistributionTbl').datagrid('reload');
			//$('#CapacityDistributionFrm  [name=id]').val(d.id);
			msApp.resetForm('capacitydistributionFrm');
		}
	}, {
		key: 'edit',
		value: function edit(index, row) {
			...
			this.MsCapacityDistributionModel.get(index, row);
		}
	}, {
		key: 'showGrid',
		...
		value: function formatDetail(value, row) {
			return '<a href="javascript:void(0)"  onClick="MsCapacityDistribution.delete(event,' + row.id + ')"><span class="btn btn-danger btn-xs"><i class="fa fa-search"></i>Delete</span></a>';
		}
	}]);

	return MsCapacityDistributionController;
}();

window.MsCapacityDistribution = new MsCapacityDistributionController(new MsCapacityDistributionModel());
MsCapacityDistribution.showGrid();
...
var MsCapacityDistributionModel = function (_MsModel) {
	_inherits(MsCapacityDistributionModel, _MsModel);

	function MsCapacityDistributionModel() {
		_classCallCheck(this, MsCapacityDistributionModel);

		return _possibleConstructorReturn(this, (MsCapacityDistributionModel.__proto__ || Object.getPrototypeOf(MsCapacityDistributionModel)).call(this));
	}

	return MsCapacityDistributionModel;
}(MsModel);

module.exports = MsCapacityDistributionModel;
