...
var MsGroupModel = __webpack_require__(61);

var MsGroupController = function () {
	function MsGroupController(MsGroupModel) {
		_classCallCheck(this, MsGroupController);

		this.MsGroupModel = MsGroupModel;
		this.formId = 'groupFrm';
		this.dataTable = '#groupTbl';
		this.route = msApp.baseUrl() + "/group";
	}

	_createClass(MsGroupController, [{
		key: 'submit',
		value: function submit() {
			var formObj = msApp.get(this.formId);
			if (formObj.id) {
				this.MsGroupModel.save(this.route + "/" + formObj.id, 'PUT', msApp.qs.stringify(formObj), this.response);
			} else {
				this.MsGroupModel.save(this.route, 'POST', msApp.qs.stringify(formObj), this.response);
			}
		}
	}, {
		key: 'remove',
		value: function remove() {
			var formObj = msApp.get(this.formId);
			this.MsGroupModel.save(this.route + "/" + formObj.id, 'DELETE', null, this.response);
		}
	}, {
		key: 'delete',
		value: function _delete(event, id) {
			event.stopPropagation();
			this.MsGroupModel.save(this.route + "/" + id, 'DELETE', null, this.response);
		}
	}, {
		key: 'response',
		value: function response(d) {
			$('#groupTbl').datagrid('reload');
		}
	}, {
		key: 'edit',
		value: function edit(index, row) {
			...
			this.MsGroupModel.get(index, row);
		}
	}, {
		key: 'showGrid',
		...
		value: function formatDetail(value, row) {
			return '<a href="javascript:void(0)"  onClick="MsGroup.delete(event,' + row.id + ')"><span class="btn btn-danger btn-xs"><i class="fa fa-search"></i>Delete</span></a>';
		}
	}]);

	return MsGroupController;
}();

window.MsGroup = new MsGroupController(new MsGroupModel());
MsGroup.showGrid();
...
var MsModel = __webpack_require__(1);

var MsGroupModel = function (_MsModel) {
	_inherits(MsGroupModel, _MsModel);

	function MsGroupModel() {
		_classCallCheck(this, MsGroupModel);

		return _possibleConstructorReturn(this, (MsGroupModel.__proto__ || Object.getPrototypeOf(MsGroupModel)).call(this));
	}

	return MsGroupModel;
}(MsModel);

module.exports = MsGroupModel;
