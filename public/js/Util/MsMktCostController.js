...
var MsMktCostModel = __webpack_require__(330);

var MsMktCostController = function () {
	function MsMktCostController(MsMktCostModel) {
		_classCallCheck(this, MsMktCostController);
	...
	_createClass(MsMktCostController, [{
		key: 'submit',
		...
		value: function edit(index, row) {
			...
			var data = this.MsMktCostModel.get(index, row);
			data.then(function (response) {
				$('#dg').datagrid({
					idField: 'id',
					data: response.data.fabric,
					columns: [[{ field: 'style_gmt', title: 'Style GMT', width: 150 }, { field: 'gmtspart', title: 'GMT Part', width: 150 }, { field: 'fabric_description', title: 'Fabric Description', width: 250 }, { field: 'fabricnature', title: 'Fabric Nature', width: 80 }, { field: 'fabriclooks', title: 'Fabric Looks', width: 80 }, { field: 'fabricshape', title: 'Fabric Shape', width: 80 }, { field: 'materialsourcing', title: 'Material Src', width: 80 }, { field: 'uom_name', title: 'UOM', width: 40 }, { field: 'gsm_weight', title: 'GSM/Weight', width: 60, editor: 'textbox' }, { field: 'cons', title: 'Cons', width: 80, align: 'center',
						formatter: function formatter(value, row, index) {
							var c = '<a href="javascript:void(0)" onclick="MsMktCostFabric.openConsWindow(' + row.id + ')">Add Cons</a> ';
							return c;
						}
					}, { field: 'rate', title: 'Rate', width: 70 }, { field: 'Amount', title: 'Amount', width: 70 }, { field: 'action', title: 'Action', width: 80, align: 'center',
						formatter: function formatter(value, row, index) {
							if (row.editing) {
								var s = '<a href="javascript:void(0)" onclick="MsMktCost.saverow(this)">Save</a> ';
								var c = '<a href="javascript:void(0)" onclick="MsMktCost.cancelrow(this)">Cancel</a>';
								return s + c;
							} else {
								var e = '<a href="javascript:void(0)" onclick="MsMktCost.editrow(this)">Edit</a> ';
								var d = '<a href="javascript:void(0)" onclick="MsMktCost.deleterow(this)">Delete</a>';
								return e + d;
							}
						}
					}]],
					...
					onAfterEdit: function onAfterEdit(index, row, changes) {
						row.editing = false;
						$(this).datagrid('refreshRow', index);
						var formObj = {};
						formObj.id = row.id;
						formObj.mkt_cost_id = row.mkt_cost_id;
						formObj.style_fabrication_id = row.style_fabrication_id;
						formObj.gsm_weight = row.gsm_weight;
						MsMktCostFabric.submit(formObj, index);
					},
		...
		key: 'getRowIndex',
		value: function getRowIndex(target) {
			var tr = $(target).closest('tr.datagrid-row');
			return parseInt(tr.attr('datagrid-row-index'));
		}
	}, {
		key: 'editrow',
		value: function editrow(target) {
			$('#dg').datagrid('beginEdit', this.getRowIndex(target));
		}
	}, {
		key: 'deleterow',
		value: function deleterow(target) {
			var self = this;
		...
		key: 'saverow',
		value: function saverow(target) {
			$('#dg').datagrid('endEdit', this.getRowIndex(target));
		}
	}, {
		key: 'cancelrow',
		value: function cancelrow(target) {
		...
		value: function calculateLeadTime() {
			var est_ship_date = $('#est_ship_date').datepicker('getDate');
			var op_date = $('#op_date').datepicker('getDate');

			$('#lead_time').val(msApp.dateDiffDays(est_ship_date, op_date));
			this.setWeek();
		}
	}, {
		key: 'setWeek',
		value: function setWeek() {
			var est_ship_date = $('#est_ship_date').datepicker('getDate');
		...
		key: 'openStyleWindow',
		value: function openStyleWindow() {
			$('#w').window('open');
		}
	}, {
		key: 'showStyleGrid',
		value: function showStyleGrid() {
			var data = {};
...
var MsMktCostModel = function (_MsModel) {
	_inherits(MsMktCostModel, _MsModel);

	function MsMktCostModel() {
		_classCallCheck(this, MsMktCostModel);

		return _possibleConstructorReturn(this, (MsMktCostModel.__proto__ || Object.getPrototypeOf(MsMktCostModel)).call(this));
	}

	return MsMktCostModel;
}(MsModel);

module.exports = MsMktCostModel;
