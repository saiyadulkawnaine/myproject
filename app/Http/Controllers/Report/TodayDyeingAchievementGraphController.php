...
class TodayDyeingAchievementGraphController extends Controller
{
    private $no_of_days;
	private $exch_rate;
	private $company;
    ...
    public function index() {
        $today=date('Y-m-d');
    	return Template::loadView('Report.TodayDyeingAchivmentGraph', ['today'=>$today]);
    }
    ...
    private function makeChart($monCusTgts,$monCusRcvs,$monCusDlvs,$dayCusTgts,$dayCusRcvs,$dayCusDlvs,$monMktTgts,$monMktRcvs,$monMktDlvs,$dayMktTgts,$dayMktRcvs,$dayMktDlvs){
        ...
        $tempdata_1="'".Template::loadView('Report.TodayDyeingAchivmentGraphData1',[
        ...
        $tempdata_2="'".Template::loadView('Report.TodayDyeingAchivmentGraphData2',[
            //'dayCusTgtQtyTot'=>$dayCusTgtQtyTot,
            'dayCusTgtAmtTot'=>$dayCusTgtAmtTot,
            //'dayCusRcvQtyTot'=>$dayCusRcvQtyTot, 
            'dayCusRcvAmtTot'=>$dayCusRcvAmtTot,
        ...
        $tempdata_3="'".Template::loadView('Report.TodayDyeingAchivmentGraphData3',[
        ...
        $tempdata_4="'".Template::loadView('Report.TodayDyeingAchivmentGraphData4',[
            //'dayCusTgtQtyTot'=>$dayCusTgtQtyTot,
    ...
    public function getGraph()
    {
        ...
        \DB::select("
            ...
            sum(so_dyeing_targets.qty*so_dyeing_targets.rate) as amount
            from
            so_dyeing_targets
            join buyers on buyers.id=so_dyeing_targets.buyer_id
            where so_dyeing_targets.execute_month>=?
        ...
        \DB::select("
            ...
            CASE
            WHEN so_dyeings.currency_id=1 
            THEN so_dyeing_fabric_rcv_items.qty*(so_dyeing_items.rate*83)
            ELSE so_dyeing_fabric_rcv_items.qty*so_dyeing_items.rate
            END as amount
            from
            ...
            where so_dyeing_fabric_rcvs.receive_date >= ?
        ...
        \DB::select("
            ...
            so_dyeing_dlv_items.grey_used as grey_used_qty,
            so_dyeing_dlv_items.rate,
            so_dyeing_dlvs.currency_id,
            CASE
            WHEN so_dyeing_dlvs.currency_id=1 
            THEN so_dyeing_dlv_items.amount*83
            ELSE so_dyeing_dlv_items.amount
            END as amount
            from 
            so_dyeing_dlvs
        ...
        \DB::select("
            select
            so_dyeing_targets.teammember_id,
            users.name as user_name,
            sum(so_dyeing_targets.qty) as qty,
            sum(so_dyeing_targets.qty*so_dyeing_targets.rate) as amount
            from
            so_dyeing_targets
            left join teammembers on teammembers.id=so_dyeing_targets.teammember_id
        ...
        \DB::select("
            ...
            users.name as user_name,
            so_dyeing_fabric_rcv_items.qty as qty,
            so_dyeings.currency_id,
            so_dyeing_items.rate,
            CASE
            WHEN so_dyeings.currency_id=1 
            ...
            so_dyeing_fabric_rcvs
            join so_dyeings on  so_dyeings.id=so_dyeing_fabric_rcvs.so_dyeing_id
            left join teammembers on  teammembers.id=so_dyeings.teammember_id
            left join users on users.id=teammembers.user_id
            join so_dyeing_fabric_rcv_items on  so_dyeing_fabric_rcv_items.so_dyeing_fabric_rcv_id=so_dyeing_fabric_rcvs.id
            join so_dyeing_refs on so_dyeing_refs.id=so_dyeing_fabric_rcv_items.so_dyeing_ref_id
            join so_dyeing_items on so_dyeing_items.so_dyeing_ref_id=so_dyeing_refs.id
