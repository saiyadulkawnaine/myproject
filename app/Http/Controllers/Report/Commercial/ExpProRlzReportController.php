...
class ExpProRlzReportController extends Controller
{
    ...
    public function index() {
        ...
        return Template::loadView('Report.Commercial.ExpProRlzReport',['company'=>$company,'buyer'=>$buyer,'bank'=>$bank]);
    }
    ...
    public function reportData()
    {
        ...
        $rows->fdr_cr=$rows->rlz_amt_exprlz_doc_value+$rows->rlz_ded_exprlz_doc_value;
    	...
    	$rows->fdr_cr=number_format($rows->fdr_cr,2);
     ...
     private function getData()
     {
        ...
        ->selectRaw('
			...
			sub_exprlz.doc_value as sub_exprlz_doc_value,
			rlz_amt_exprlz.doc_value as rlz_amt_exprlz_doc_value,
			rlz_ded_exprlz.doc_value as rlz_ded_exprlz_doc_value
		')
		...
		->leftJoin(\DB::raw("(
			...
			exp_doc_sub_transections
			join commercial_heads on commercial_heads.id=exp_doc_sub_transections.commercialhead_id
			where commercial_heads.commercialhead_type_id=26
			group by
			exp_doc_sub_transections.exp_doc_submission_id) sub_exprlz"), "sub_exprlz.exp_doc_submission_id", "=", "exp_doc_submissions.id")
			...
			and exp_pro_rlz_amounts.deleted_at is null
			group by
			exp_pro_rlzs.exp_doc_submission_id) rlz_amt_exprlz"), "rlz_amt_exprlz.exp_doc_submission_id", "=", "exp_doc_submissions.id")
			...
			and exp_pro_rlz_deducts.deleted_at is null
			group by
			exp_pro_rlzs.exp_doc_submission_id) rlz_ded_exprlz"), "rlz_ded_exprlz.exp_doc_submission_id", "=", "exp_doc_submissions.id")
		
		->when(request('date_from'), function ($q) {
			return $q->where('exp_pro_rlzs.realization_date', '>=',request('date_from', 0));
		})
		->when(request('date_to'), function ($q) {
			return $q->where('exp_pro_rlzs.realization_date', '<=',request('date_to', 0));
		})
