...
class YarnStockKnitingPartyController extends Controller
{
  ...
  public function index() {
    ...
    return Template::loadView('Report.Inventory.YarnStockKnitingParty',[]);
  }
  public function reportData() {
        ...
        $results = collect(
          \DB::select("
              ...
              left join (
                ...
                sum(prod_knit_item_yarns.qty) as qty,
                sum(prod_knit_item_yarns.amount) as amount
                from
                prod_knits
                join prod_knit_items on prod_knits.id=prod_knit_items.prod_knit_id
                join prod_knit_item_yarns on prod_knit_items.id=prod_knit_item_yarns.prod_knit_item_id
                where
                prod_knits.prod_date < ? 
                and prod_knits.deleted_at is null
                and prod_knit_items.deleted_at is null
                and prod_knit_item_yarns.deleted_at is null
                group by prod_knits.supplier_id
              ...
              where 
              supplier_natures.contact_nature_id=26
    ...
    public function issueDtl(){
        ...
        $results = collect(
          \DB::select("
            ...
            and inv_isus.issue_date <= ?
            $supplierCond
            and supplier_natures.contact_nature_id=26
            and supplier_natures.deleted_at is null
            and inv_yarn_transactions.deleted_at is null
            and inv_yarn_isu_items.deleted_at is null
            and inv_isus.deleted_at is null
    ...
    public function usedDtl(){
        ...
        $supplierCond='';
        if($supplier_id){
          $supplierCond= ' and prod_knits.supplier_id= '.$supplier_id;
        }
        else{
          $supplierCond= '';
        }
        ...
        $results = collect(
          \DB::select("
            ...
            join prod_knit_items on prod_knits.id=prod_knit_items.prod_knit_id
            join prod_knit_item_yarns on prod_knit_items.id=prod_knit_item_yarns.prod_knit_item_id
            join inv_yarn_isu_items on inv_yarn_isu_items.id=prod_knit_item_yarns.inv_yarn_isu_item_id
            join inv_yarn_items on inv_yarn_items.id=inv_yarn_isu_items.inv_yarn_item_id
            join item_accounts on item_accounts.id=inv_yarn_items.item_account_id
            join yarncounts on yarncounts.id=item_accounts.yarncount_id
            join yarntypes on yarntypes.id=item_accounts.yarntype_id
            ...
            join style_fabrications on style_fabrications.id=budget_fabrics.style_fabrication_id
            left join colorranges on colorranges.id=po_knit_service_item_qties.colorrange_id
            ) pl_sales_orders on pl_sales_orders.id=prod_knit_items.pl_knit_item_id
            left join (
            select po_knit_service_item_qties.id,
            sales_orders.sale_order_no as sale_order_no_po,
            styles.style_ref as style_ref_po,
            ...
            join style_fabrications on style_fabrications.id=budget_fabrics.style_fabrication_id
            left join colorranges on colorranges.id=po_knit_service_item_qties.colorrange_id
            ) po_sales_orders on po_sales_orders.id=prod_knit_items.po_knit_service_item_qty_id
            where
            prod_knits.prod_date>= ?
            and prod_knits.prod_date<= ?
            $supplierCond 
        ...
        ->map(function($results) use($yarnDropdown){
          $results->yarn_desc=$yarnDropdown[$results->item_account_id];
          $results->yarn_count=$results->count."/".$results->symbol;
          $results->prod_date=date('d-M-Y',strtotime($results->prod_date));
          $results->rate=0;
          if($results->qty){
          $results->rate=$results->amount/$results->qty;
          }
