...
class ProdAopFinDailyLoadReportController extends Controller {
    private $prodknit;
        ...
        ) 
    {
      ...
      //$this->middleware('permission:view.prodaopfindailyloadreports',   ['only' => ['create', 'index','show']]);
    }
    ...
    public function index() {
        ...
        $process_name=array_prepend(array_pluck($this->productionprocess->whereIn('production_area_id',[25,30])->get(),'process_name','id'),'','');
        return Template::loadView('Report.FabricProduction.ProdAopFinDailyLoadReport',['supplier'=>$supplier,'date_from'=>$date_from,'date_to'=>$date_to,'process_name'=>$process_name]);
    }
    ...
    public function reportData(){
        ...
        $process_name=array_prepend(array_pluck($this->productionprocess->get(),'process_name','id'),'','');
        ...
        $rows=collect(
            ...
            left join (
                ...
                (select
                    ...
                    join prod_aop_batches on prod_aop_batches.id=prod_batch_finish_progs.prod_aop_batch_id
                    left join (
                        select
                        prod_batch_finish_prog_rolls.prod_batch_finish_prog_id,
                        sum(prod_aop_batch_rolls.qty) as roll_qty
                        from
            ...
            left join(
                ...
                where
                asset_acquisitions.production_area_id in (25,30)
            ...
            where asset_acquisitions.production_area_id in (25,30)
        ...
        ->map(function($rows) use($process_name){
            $rows->production_process_id=isset($rows->production_process_id)?$process_name[$rows->production_process_id]:'--';
            $now=Carbon::now();
