...
class BatchReportController extends Controller
{
    ...
    public function index() {
        ...
        return Template::loadView('Report.FabricProduction.BatchReport',['company'=>$company,'buyer'=>$buyer]);
    }
    public function reportData()
    {
        ...
        ->when(request('target_date_from'), function ($q) {
        return $q->where('prod_batches.target_load_date', '>=', request('target_date_from', 0));
        })
        ->when(request('target_date_to'), function ($q) {
        return $q->where('prod_batches.target_load_date', '<=', request('target_date_to', 0));
        })
        ...
        ->get([
            ...
            'asset_acquisitions.brand as machine_brand',
        ...
        ->map(function($rows) use($batchfor,$yesno){
            ...
            $rows->target_load_date=$rows->target_load_date?date('d-M-Y',strtotime($rows->target_load_date)):'--';
            $rows->redyeing=$yesno[$rows->is_redyeing];
            $rows->approval_status=$rows->approved_by?'Approved':'Not';
            $rows->prod_capacity_var=$rows->prod_capacity-$rows->batch_wgt;
            $rows->batch_wgt=number_format($rows->batch_wgt,2);
            $rows->prod_capacity=number_format($rows->prod_capacity,2);
            $rows->prod_capacity_var=number_format($rows->prod_capacity_var,2);
            return $rows;
        });
