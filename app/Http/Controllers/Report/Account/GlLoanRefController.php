...
class GlLoanRefController extends Controller
{
	private $company;
	private $accyear;
	private $buyer;
	...
	public function __construct(CompanyRepository $company,AccYearRepository $accyear,BuyerRepository $buyer,SupplierRepository $supplier,AccChartLocationRepository $location,AccChartDivisionRepository $division,AccChartDepartmentRepository $department,AccChartSectionRepository $section,EmployeeRepository $employee)
    {
		...
		$this->middleware('permission:view.glloanref',   ['only' => ['create', 'index','show']]);
    }
    public function index() {
		$company=array_prepend(array_pluck($this->company->get(),'name','id'),'-Select-','');
		$accyear=array_prepend(array_pluck($this->accyear->get(),'name','id'),'-Select-','');
		
      return Template::loadView('Report.Account.GlLoanRef',['company'=>$company,'accyear'=>$accyear]);
    }
    ...
    public function getDateRange(){
     ...
     private function getData()
     {
		...
		if($start_date < $yearStart){
			...
			$opening = DB::table("acc_trans_loan_refs")
			->selectRaw('sum(acc_trans_loan_refs.amount) as amount, acc_chart_ctrl_heads.code,acc_chart_ctrl_heads.name as acc_ctrl_head_name,acc_trans_loan_refs.loan_ref_no')
			->join('acc_trans_prnts',function($join){
			$join->on('acc_trans_prnts.id','=','acc_trans_loan_refs.acc_trans_prnt_id');
			})
			...
			$join->on('acc_chart_ctrl_heads.id','=','acc_trans_loan_refs.acc_chart_ctrl_head_id');
			})
			...
			->when(request('loan_ref_name'), function ($q) use ($idarray) {
			return $q->where('acc_trans_loan_refs.loan_ref_no','like','%'.request('loan_ref_name', 0).'%');
		    })
			->whereNull('acc_trans_loan_refs.deleted_at')
			->groupBy('acc_chart_ctrl_heads.code')
			->groupBy('acc_trans_loan_refs.loan_ref_no')
			->groupBy('acc_chart_ctrl_heads.name')
			->get()
			->groupBy(['loan_ref_no','code'])->toArray();
			//->pluck('amount','code');
		...
		$data = DB::table("acc_trans_loan_refs")
		...
		"acc_trans_loan_refs.amount",
		
		"acc_trans_loan_refs.loan_ref_no",
		'acc_periods.end_date',
		'acc_periods.period',
		'acc_trans_loan_refs.chld_narration'
		)
		->join('acc_trans_prnts',function($join){
		$join->on('acc_trans_prnts.id','=','acc_trans_loan_refs.acc_trans_prnt_id');
		})
        ...
        $data->loan_ref_name = $data->loan_ref_no;//isset($otherPartise[$data->supplier_id])?$otherPartise[$data->supplier_id]:'';
		...
		$grouped = $data->groupBy(['loan_ref_no','account']);
	...
	public function html() {
		return Template::loadView('Report.Account.GlDataLoanRef',$this->getData());
    }
	...
	$view= \View::make('Defult.Report.Account.GlDataPdfLoanRef',$data);
	$html_content=$view->render();
	$pdf->SetY(40);
	$pdf->WriteHtml($html_content, true, false,true,false,'');
    $filename = storage_path() . '/glLoanRefPdf.pdf';
