...
class GlBuyController extends Controller
{
	private $company;
	private $accyear;
	private $buyer;
    ...
    public function index() {
      ...
      return Template::loadView('Report.Account.GlBuy',['company'=>$company,'accyear'=>$accyear]);
    }
    ...
    public function getDateRange(){
     ...
     private function getData()
     {
		...
		if($start_date < $yearStart){
			...
			$opening = DB::table("acc_trans_sales")
			->selectRaw('sum(acc_trans_sales.amount) as amount, acc_chart_ctrl_heads.code,acc_chart_ctrl_heads.name as acc_ctrl_head_name,acc_trans_sales.buyer_id')
			...
			->groupBy('acc_trans_sales.buyer_id')
			->groupBy('acc_chart_ctrl_heads.name')
			->get()
			->groupBy(['buyer_id','code'])->toArray();
			//->pluck('amount','code');
		...
		"acc_trans_sales.amount",
		"acc_trans_sales.bill_no",
		"acc_trans_sales.buyer_id",
		'acc_periods.end_date',
		'acc_periods.period',
		'acc_trans_sales.chld_narration'
		)
		->join('acc_trans_prnts',function($join){
		$join->on('acc_trans_prnts.id','=','acc_trans_sales.acc_trans_prnt_id');
		})
		...
		->get()
		->map(function ($data) use ($journalType,$buyer,$employee) {
		if($data->amount < 0 ){
        ...
        $data->buyer_name =isset($buyer[$data->buyer_id])?$buyer[$data->buyer_id]:'';
		...
		$grouped = $data->groupBy(['buyer_id','account']);

		$grouped->toArray();
		return ['data'=>$grouped,'opening'=>$opening,'company'=>$company,'start_date'=>$start_date,'end_date'=>$end_date,'buyer'=>$buyer];

     }
	public function html() {
		return Template::loadView('Report.Account.GlDataBuy',$this->getData());
    }
   ...
   // $image_file = url('/').'/images/logo/'.$data['company']->logo;
	...
	$view= \View::make('Defult.Report.Account.GlDataPdfBuy',$data);
	$html_content=$view->render();
	$pdf->SetY(42);
	$pdf->WriteHtml($html_content, true, false,true,false,'');
    $filename = storage_path() . '/glBuyPdf.pdf';
