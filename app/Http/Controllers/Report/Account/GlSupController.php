...
class GlSupController extends Controller
{
	private $company;
	private $accyear;
	private $buyer;
	...
	public function __construct(CompanyRepository $company,AccYearRepository $accyear,BuyerRepository $buyer,SupplierRepository $supplier,AccChartLocationRepository $location,AccChartDivisionRepository $division,AccChartDepartmentRepository $department,AccChartSectionRepository $section,EmployeeRepository $employee)
    {
		...
		$this->middleware('permission:view.glsup',   ['only' => ['create', 'index','show']]);
    }
    public function index() {
		$company=array_prepend(array_pluck($this->company->get(),'name','id'),'-Select-','');
		$accyear=array_prepend(array_pluck($this->accyear->get(),'name','id'),'-Select-','');
		
      return Template::loadView('Report.Account.GlSup',['company'=>$company,'accyear'=>$accyear]);
    }
    ...
    public function getDateRange(){
     ...
     private function getData()
     {
		...
		if($start_date < $yearStart){
			return response()->json(array('success' => false,'message' => 'Start Date is not in this year'),200);
		}
			...
			$opening = DB::table("acc_trans_purchases")
			->selectRaw('sum(acc_trans_purchases.amount) as amount, acc_chart_ctrl_heads.code,acc_chart_ctrl_heads.name as acc_ctrl_head_name,acc_trans_purchases.supplier_id')
			...
			->groupBy('acc_trans_purchases.supplier_id')
		...
		$data = DB::table("acc_trans_purchases")
		...
		"acc_trans_purchases.amount",
		"acc_trans_purchases.bill_no",
		"acc_trans_purchases.supplier_id",
		'acc_periods.end_date',
		'acc_periods.period',
		'acc_trans_purchases.chld_narration'
		)
		->join('acc_trans_prnts',function($join){
		$join->on('acc_trans_prnts.id','=','acc_trans_purchases.acc_trans_prnt_id');
		})
		...
		->get()
		->map(function ($data) use ($journalType,$supplier,$employee) {
			if($data->amount < 0 ){
	        ...
	        $data->supplier_name =isset($supplier[$data->supplier_id])?$supplier[$data->supplier_id]:'';
		...
		$grouped->toArray();
		return ['data'=>$grouped,'opening'=>$opening,'company'=>$company,'start_date'=>$start_date,'end_date'=>$end_date,'supplier'=>$supplier];

     }
	public function html() {
		return Template::loadView('Report.Account.GlDataSup',$this->getData());
    }
    ...
    public function pdf() {
		...
		$view= \View::make('Defult.Report.Account.GlDataPdfSup',$data);
		$html_content=$view->render();
		$pdf->SetY(40);
		$pdf->WriteHtml($html_content, true, false,true,false,'');
	    $filename = storage_path() . '/glSupPdf.pdf';
