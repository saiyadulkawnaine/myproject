...
class GlOtpController extends Controller
{
	...
	public function __construct(CompanyRepository $company,AccYearRepository $accyear,BuyerRepository $buyer,SupplierRepository $supplier,AccChartLocationRepository $location,AccChartDivisionRepository $division,AccChartDepartmentRepository $department,AccChartSectionRepository $section,EmployeeRepository $employee)
    {
		...
		$this->middleware('permission:view.glotp',   ['only' => ['create', 'index','show']]);
    }
    public function index() {
		$company=array_prepend(array_pluck($this->company->get(),'name','id'),'-Select-','');
		$accyear=array_prepend(array_pluck($this->accyear->get(),'name','id'),'-Select-','');
		
      return Template::loadView('Report.Account.GlOtp',['company'=>$company,'accyear'=>$accyear]);
    }
    ...
    public function getDateRange(){
     ...
     private function getData()
     {
		...
		if($start_date < $yearStart){
			...
			$opening = DB::table("acc_trans_other_parties")
			->selectRaw('sum(acc_trans_other_parties.amount) as amount, acc_chart_ctrl_heads.code,acc_chart_ctrl_heads.name as acc_ctrl_head_name,acc_trans_other_parties.supplier_id')
			->join('acc_trans_prnts',function($join){
			$join->on('acc_trans_prnts.id','=','acc_trans_other_parties.acc_trans_prnt_id');
			})
			...
			$join->on('acc_chart_ctrl_heads.id','=','acc_trans_other_parties.acc_chart_ctrl_head_id');
			})
			...
			return $q->whereIn('acc_trans_other_parties.supplier_id',$idarray);
		    })
			->whereNull('acc_trans_other_parties.deleted_at')
			->groupBy('acc_chart_ctrl_heads.code')
			->groupBy('acc_trans_other_parties.supplier_id')
			->groupBy('acc_chart_ctrl_heads.name')
			->get()
			->groupBy(['supplier_id','code'])->toArray();
			//->pluck('amount','code');
		...
		$data = DB::table("acc_trans_other_parties")
		...
		"acc_trans_other_parties.amount",
		"acc_trans_other_parties.bill_no",
		"acc_trans_other_parties.supplier_id",
		'acc_periods.end_date',
		'acc_periods.period',
		'acc_trans_other_parties.chld_narration'
		)
		->join('acc_trans_prnts',function($join){
		$join->on('acc_trans_prnts.id','=','acc_trans_other_parties.acc_trans_prnt_id');
		})
        ...
        $data->other_party_name =isset($otherPartise[$data->supplier_id])?$otherPartise[$data->supplier_id]:'';
		...
		$grouped = $data->groupBy(['supplier_id','account']);
	...
	public function html() {
		return Template::loadView('Report.Account.GlDataOtp',$this->getData());
    }
	...
	$view= \View::make('Defult.Report.Account.GlDataPdfOtp',$data);
	$html_content=$view->render();
	$pdf->SetY(42);
	$pdf->WriteHtml($html_content, true, false,true,false,'');
    $filename = storage_path() . '/glOtpPdf.pdf';
