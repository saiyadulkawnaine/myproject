...
use App\Library\Sms;
class BudgetTrimApprovalController extends Controller
{
    private $budget;
    ...
    public function __construct(
        ...
        $this->middleware('permission:approvefirst.budgettrim',   ['only' => ['firstapproved']]);
        $this->middleware('permission:approvesecond.budgettrim', ['only' => ['secondapproved']]);
        $this->middleware('permission:approvethird.budgettrim',   ['only' => ['thirdapproved']]);
        $this->middleware('permission:approvefinal.budgettrim', ['only' => ['finalapproved']]);
        $this->middleware('permission:approvefirst.budgettrim|approvesecond.budgettrim|approvethird.budgettrim|approvefinal.budgettrim', ['only' => ['approvalReturn']]);
	...
	public function reportData() {
        ...
        ->when($approval_type_id, function ($q) use ($approval_type_id){
            if($approval_type_id==1){
                return $q->whereNull('budget_approvals.trim_first_approved_at');
            }
            if($approval_type_id==2){
            return $q->whereNotNull('budget_approvals.trim_first_approved_at')->whereNull('budget_approvals.trim_second_approved_at');
            }
            if($approval_type_id==3){
            return $q->whereNotNull('budget_approvals.trim_second_approved_at')->whereNull('budget_approvals.trim_third_approved_at');
            }
            if($approval_type_id==10){
            return $q->whereNotNull('budget_approvals.trim_third_approved_at')->whereNull('budget_approvals.trim_final_approved_at');
            }
        })
    ...
    public function firstapproved (Request $request)
    {
        ...
        foreach($request->id as $index=>$id){
            if($id){
                ...
                $user = \Auth::user();
                $trim_first_approved_at=date('Y-m-d h:i:s');
                $master->trim_first_approved_by=$user->id;
                $master->trim_first_approved_at=$trim_first_approved_at;
    ...
    public function secondapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $trim_second_approved_at=date('Y-m-d h:i:s');
                $master->trim_second_approved_by=$user->id;
                $master->trim_second_approved_at=$trim_second_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
    ...
    public function thirdapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $trim_third_approved_at=date('Y-m-d h:i:s');
                $master->trim_third_approved_by=$user->id;
                $master->trim_third_approved_at=$trim_third_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
    ...
    public function finalapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $trim_final_approved_at=date('Y-m-d h:i:s');
                $master->trim_final_approved_by=$user->id;
                $master->trim_final_approved_at=$trim_final_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
