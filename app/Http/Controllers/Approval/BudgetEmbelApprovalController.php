...
use App\Library\Sms;
class BudgetEmbelApprovalController extends Controller
{
    private $budget;
    ...
    public function __construct(
        ...
        $this->middleware('permission:approvefirst.budgetembel',   ['only' => ['firstapproved']]);
        $this->middleware('permission:approvesecond.budgetembel', ['only' => ['secondapproved']]);
        $this->middleware('permission:approvethird.budgetembel',   ['only' => ['thirdapproved']]);
        $this->middleware('permission:approvefinal.budgetembel', ['only' => ['finalapproved']]);
        $this->middleware('permission:approvefirst.budgetembel|approvesecond.budgetembel|approvethird.budgetembel|approvefinal.budgetembel', ['only' => ['approvalReturn']]);
	...
	public function reportData() {
        ...
        ->when($approval_type_id, function ($q) use ($approval_type_id){
            if($approval_type_id==1){
                return $q->whereNull('budget_approvals.embel_first_approved_at');
            }
            if($approval_type_id==2){
            return $q->whereNotNull('budget_approvals.embel_first_approved_at')->whereNull('budget_approvals.embel_second_approved_at');
            }
            if($approval_type_id==3){
            return $q->whereNotNull('budget_approvals.embel_second_approved_at')->whereNull('budget_approvals.embel_third_approved_at');
            }
            if($approval_type_id==10){
            return $q->whereNotNull('budget_approvals.embel_third_approved_at')->whereNull('budget_approvals.embel_final_approved_at');
            }
        })
    ...
    public function firstapproved (Request $request)
    {
        ...
        foreach($request->id as $index=>$id){
            if($id){
                ...
                $user = \Auth::user();
                $embel_first_approved_at=date('Y-m-d h:i:s');
                $master->embel_first_approved_by=$user->id;
                $master->embel_first_approved_at=$embel_first_approved_at;
    ...
    public function secondapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $embel_second_approved_at=date('Y-m-d h:i:s');
                $master->embel_second_approved_by=$user->id;
                $master->embel_second_approved_at=$embel_second_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
    ...
    public function thirdapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $embel_third_approved_at=date('Y-m-d h:i:s');
                $master->embel_third_approved_by=$user->id;
                $master->embel_third_approved_at=$embel_third_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
    ...
    public function finalapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $embel_final_approved_at=date('Y-m-d h:i:s');
                $master->embel_final_approved_by=$user->id;
                $master->embel_final_approved_at=$embel_final_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
