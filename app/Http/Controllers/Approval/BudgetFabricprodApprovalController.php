...
use App\Library\Sms;
class BudgetFabricprodApprovalController extends Controller
{
    private $budget;
    ...
    public function __construct(
        ...
        $this->middleware('permission:approvefirst.budgetfabricprod',   ['only' => ['firstapproved']]);
        $this->middleware('permission:approvesecond.budgetfabricprod', ['only' => ['secondapproved']]);
        $this->middleware('permission:approvethird.budgetfabricprod',   ['only' => ['thirdapproved']]);
        $this->middleware('permission:approvefinal.budgetfabricprod', ['only' => ['finalapproved']]);
        $this->middleware('permission:approvefirst.budgetfabricprod|approvesecond.budgetfabricprod|approvethird.budgetfabricprod|approvefinal.budgetfabricprod', ['only' => ['approvalReturn']]);
	...
	public function reportData() {
        ...
        ->when($approval_type_id, function ($q) use ($approval_type_id){
            if($approval_type_id==1){
                return $q->whereNull('budget_approvals.fabricprod_first_approved_at');
            }
            if($approval_type_id==2){
            return $q->whereNotNull('budget_approvals.fabricprod_first_approved_at')->whereNull('budget_approvals.fabricprod_second_approved_at');
            }
            if($approval_type_id==3){
            return $q->whereNotNull('budget_approvals.fabricprod_second_approved_at')->whereNull('budget_approvals.fabricprod_third_approved_at');
            }
            if($approval_type_id==10){
            return $q->whereNotNull('budget_approvals.fabricprod_third_approved_at')->whereNull('budget_approvals.fabricprod_final_approved_at');
            }
        })
    ...
    public function firstapproved (Request $request)
    {
        ...
        foreach($request->id as $index=>$id){
            if($id){
                ...
                $user = \Auth::user();
                $fabricprod_first_approved_at=date('Y-m-d h:i:s');
                $master->fabricprod_first_approved_by=$user->id;
                $master->fabricprod_first_approved_at=$fabricprod_first_approved_at;
    ...
    public function secondapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $fabricprod_second_approved_at=date('Y-m-d h:i:s');
                $master->fabricprod_second_approved_by=$user->id;
                $master->fabricprod_second_approved_at=$fabricprod_second_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
    ...
    public function thirdapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $fabricprod_third_approved_at=date('Y-m-d h:i:s');
                $master->fabricprod_third_approved_by=$user->id;
                $master->fabricprod_third_approved_at=$fabricprod_third_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
    ...
    public function finalapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $fabricprod_final_approved_at=date('Y-m-d h:i:s');
                $master->fabricprod_final_approved_by=$user->id;
                $master->fabricprod_final_approved_at=$fabricprod_final_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
    ...
    public function approvalReturn(Request $request){
        ...
        $this->approvalcommenthistory->create([
                'model_id'=>$id,
                'model_type'=>'budget_fabric_prods',
