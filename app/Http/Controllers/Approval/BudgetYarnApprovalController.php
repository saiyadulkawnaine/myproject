...
use App\Library\Sms;
class BudgetYarnApprovalController extends Controller
{
    private $budget;
    ...
    public function __construct(
        ...
        $this->middleware('permission:approvefirst.budgetyarn',   ['only' => ['firstapproved']]);
        $this->middleware('permission:approvesecond.budgetyarn', ['only' => ['secondapproved']]);
        $this->middleware('permission:approvethird.budgetyarn',   ['only' => ['thirdapproved']]);
        $this->middleware('permission:approvefinal.budgetyarn', ['only' => ['finalapproved']]);
        $this->middleware('permission:approvefirst.budgetyarn|approvesecond.budgetyarn|approvethird.budgetyarn|approvefinal.budgetyarn', ['only' => ['approvalReturn']]);
	...
	public function reportData() {
        ...
        ->when($approval_type_id, function ($q) use ($approval_type_id){
            if($approval_type_id==1){
                return $q->whereNull('budget_approvals.yarn_first_approved_at');
            }
            if($approval_type_id==2){
            return $q->whereNotNull('budget_approvals.yarn_first_approved_at')->whereNull('budget_approvals.yarn_second_approved_at');
            }
            if($approval_type_id==3){
            return $q->whereNotNull('budget_approvals.yarn_second_approved_at')->whereNull('budget_approvals.yarn_third_approved_at');
            }
            if($approval_type_id==10){
            return $q->whereNotNull('budget_approvals.yarn_third_approved_at')->whereNull('budget_approvals.yarn_final_approved_at');
            }
        })
    ...
    public function firstapproved (Request $request)
    {
        ...
        foreach($request->id as $index=>$id){
            if($id){
                ...
                $user = \Auth::user();
                $yarn_first_approved_at=date('Y-m-d h:i:s');
                $master->yarn_first_approved_by=$user->id;
                $master->yarn_first_approved_at=$yarn_first_approved_at;
    ...
    public function secondapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $yarn_second_approved_at=date('Y-m-d h:i:s');
                $master->yarn_second_approved_by=$user->id;
                $master->yarn_second_approved_at=$yarn_second_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
    ...
    public function thirdapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $yarn_third_approved_at=date('Y-m-d h:i:s');
                $master->yarn_third_approved_by=$user->id;
                $master->yarn_third_approved_at=$yarn_third_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
    ...
    public function finalapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $yarn_final_approved_at=date('Y-m-d h:i:s');
                $master->yarn_final_approved_by=$user->id;
                $master->yarn_final_approved_at=$yarn_final_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
