...
class MktCostFirstApprovalController extends Controller
{
	private $mktcost;
	private $company;
	private $buyer;
    ...
    public function index() {
        ...
        return Template::loadView('Approval.MktCostFirstApproval',['company'=>$company,'buyer'=>$buyer,'team'=>$team,'teammember'=>$teammember]);
    ...
    public function approved (Request $request)
    {
        ...
        $master->first_approved_by=$user->id;
        $master->second_approved_by=$user->id;
        $master->third_approved_by=$user->id;
        $master->final_approved_by=$user->id;
        $master->first_approved_at=$approved_at;
        $master->second_approved_at=$approved_at;
        $master->third_approved_at=$approved_at;
        $master->final_approved_at=$approved_at;
  ...
  //   	foreach($request->id as $index=>$id){
		...
		// 		$mktcost = $this->mktcost->update($id,
