...
class PoDyeingServiceShortApprovalController extends Controller
{
 ...
 public function __construct(
  ...
  // $this->middleware('permission:approve.podyeingserviceshorts',   ['only' => ['approved', 'index', 'reportData', 'reportDataApp', 'unapproved']]);
 }
 public function index()
 {
  $company = array_prepend(array_pluck($this->company->get(), 'name', 'id'), '-Select-', '');
  $supplier = array_prepend(array_pluck($this->supplier->get(), 'name', 'id'), '-Select-', '');
  return Template::loadView('Approval.PoDyeingServiceShortApproval', ['company' => $company, 'supplier' => $supplier]);
 }
 public function reportData()
 {
  ...
  $rows = $this->podyeingservice
   ...
   ->when(request('supplier_id'), function ($q) {
    return $q->where('po_dyeing_services.supplier_id', '=', request('supplier_id', 0));
   })
   ...
   ->whereNull('po_dyeing_services.approved_at')
   ->where([['po_dyeing_services.po_type_id', '=', 2]])
 ...
 {
  $id = request('id', 0);
  $master = $this->podyeingservice->find($id);
  $user = \Auth::user();
 ...
 public function getDyeingServiceSummery()
 {
  $id = request('id', 0);
  ...
  $details = collect(
   \DB::Select("
          ...
          po_dyeing_service_items.qty as po_qty,
          po_dyeing_service_items.rate as po_rate,
          po_dyeing_service_items.amount as po_amount,
          ...
          left join (
              select 
              po_dyeing_service_items.budget_fabric_prod_id,
              sum(po_dyeing_service_items.qty) as qty,
              avg(po_dyeing_service_items.rate) as rate,
              sum(po_dyeing_service_items.amount) as amount
              from po_dyeing_service_items 
              join budget_fabric_prods on budget_fabric_prods.id=po_dyeing_service_items.budget_fabric_prod_id
              group by 
              po_dyeing_service_items.budget_fabric_prod_id
          ...
          where (po_dyeing_services.id =  ? ) --27466
          and po_dyeing_services.deleted_at is null
        ", [$id])
  )
  ...
  $responsible = $this->podyeingservice
   ->join('po_dyeing_service_items', function ($join) {
    $join->on('po_dyeing_services.id', '=', 'po_dyeing_service_items.po_dyeing_service_id');
   })
   ->join('po_dyeing_service_item_resps', function ($join) {
    $join->on('po_dyeing_service_items.id', '=', 'po_dyeing_service_item_resps.po_dyeing_service_item_id');
   })
   ->join('sections', function ($join) {
    $join->on('sections.id', '=', 'po_dyeing_service_item_resps.section_id');
   })
   ->leftJoin('employee_h_rs', function ($join) {
    $join->on('employee_h_rs.id', '=', 'po_dyeing_service_item_resps.employee_h_r_id');
   })
   ->leftJoin('companies', function ($join) {
    $join->on('companies.id', '=', 'employee_h_rs.company_id');
   })
   ->where([['po_dyeing_services.id', '=', $id]])
   ->orderBy('po_dyeing_service_item_resps.id', 'desc')
   ->get([
    'po_dyeing_service_item_resps.*',
  ...
  return Template::loadView('Approval.PoDyeingServiceShortApprovalSummeryMatrix', ['data' => $data]);
 }
 ...
 public function poDetails()
 {
  ...
  $rows = $this->podyeingservice
   ->selectRaw('
            ...
            po_dyeing_services.source_id,
            po_dyeing_services.pay_mode,
            ...
            avg(po_dyeing_service_items.rate) as po_rate,
            sum(po_dyeing_service_items.amount) as po_amount
        ')
   ->join('companies', function ($join) {
    $join->on('companies.id', '=', 'po_dyeing_services.company_id');
   })
   ...
   ->where([['po_dyeing_service_items.budget_fabric_prod_id', '=', request('budget_fabric_prod_id', 0)]])
