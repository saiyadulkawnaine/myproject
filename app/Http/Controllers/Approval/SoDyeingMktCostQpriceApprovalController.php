...
class SoDyeingMktCostQpriceApprovalController extends Controller
{
	...
	)
    {
		...
		$this->middleware('permission:approvefirst.sodyeingmktcostqprices',   ['only' => ['firstapproved']]);
        $this->middleware('permission:approvesecond.sodyeingmktcostqprices', ['only' => ['secondapproved']]);
        $this->middleware('permission:approvethird.sodyeingmktcostqprices',   ['only' => ['thirdapproved']]);
        $this->middleware('permission:approvefinal.sodyeingmktcostqprices', ['only' => ['finalapproved']]);
    }
    ...
    public function index() {
        ...
        return Template::loadView('Approval.SoDyeingMktCostQpriceApproval',['company'=>$company,'buyer'=>$buyer,'team'=>$team,'teammember'=>$teammember]);
		
    }

	public function reportData() {
		$approval_type_id=request('approval_type_id');
		$data = $this->sodyeingmktcostqprice
		->selectRaw('
			so_dyeing_mkt_cost_qprices.id,
			so_dyeing_mkt_cost_qprices.qprice_no,
			so_dyeing_mkt_cost_qprices.qprice_date,
			so_dyeing_mkt_cost_qprices.remarks,
			buyers.code as buyer_name,
			users.name as user_name,
			so_dyeing_mkt_costs.id as so_dyeing_mkt_cost_id,
			so_dyeing_mkt_costs.costing_date,
            ...
            sum(so_dyeing_mkt_cost_qpricedtls.cost_per_kg) as cost_per_kg,
            sum(so_dyeing_mkt_cost_qpricedtls.quoted_price_bdt) as quoted_price_bdt,
            sum(so_dyeing_mkt_cost_qpricedtls.quoted_price) as quoted_price,
            sum(so_dyeing_mkt_cost_qpricedtls.profit_amount_bdt) as profit_amount_bdt,
            sum(so_dyeing_mkt_cost_qpricedtls.profit_amount) as profit_amount,
            avg(so_dyeing_mkt_cost_qpricedtls.profit_per) as profit_per
		')
        ...
        ->leftJoin(\DB::raw("(
            SELECT 
			so_dyeing_mkt_cost_qpricedtls.id,
			sum(so_dyeing_mkt_cost_fab_items.amount) as dyes_cost 
			FROM so_dyeing_mkt_cost_qpricedtls
			join so_dyeing_mkt_cost_fabs on so_dyeing_mkt_cost_fabs.id=so_dyeing_mkt_cost_qpricedtls.so_dyeing_mkt_cost_fab_id
			join so_dyeing_mkt_cost_fab_items on so_dyeing_mkt_cost_fab_items.so_dyeing_mkt_cost_fab_id=so_dyeing_mkt_cost_fabs.id
			...
			group by 
			so_dyeing_mkt_cost_qpricedtls.id
        ) fabricItemDyesCost"), "fabricItemDyesCost.id", "=", "so_dyeing_mkt_cost_qpricedtls.id")
        ->leftJoin(\DB::raw("(
            SELECT 
			so_dyeing_mkt_cost_qpricedtls.id,
			...
			group by 
			so_dyeing_mkt_cost_qpricedtls.id
        ) fabricItemChemicalCost"), "fabricItemChemicalCost.id", "=", "so_dyeing_mkt_cost_qpricedtls.id")
        ->leftJoin(\DB::raw("(
            SELECT 
			so_dyeing_mkt_cost_qpricedtls.id,
			...
			FROM so_dyeing_mkt_cost_qpricedtls
			join so_dyeing_mkt_cost_fabs on so_dyeing_mkt_cost_fabs.id=so_dyeing_mkt_cost_qpricedtls.so_dyeing_mkt_cost_fab_id
			join so_dyeing_mkt_cost_fab_fins on so_dyeing_mkt_cost_fab_fins.so_dyeing_mkt_cost_fab_id=so_dyeing_mkt_cost_fabs.id
			where  so_dyeing_mkt_cost_fab_fins.deleted_at is null
			group by 
			so_dyeing_mkt_cost_qpricedtls.id
        ) fabricSpecialFinishCost"), "fabricSpecialFinishCost.id", "=", "so_dyeing_mkt_cost_qpricedtls.id")
		->leftJoin(\DB::raw("(
            select 
			so_dyeing_mkt_cost_qpricedtls.id,
			sum(so_dyeing_mkt_cost_fabs.fabric_wgt) as fabric_wgt, 
			sum(so_dyeing_mkt_cost_fabs.overhead_amount) as overhead_amount 
			from so_dyeing_mkt_cost_qpricedtls
			join so_dyeing_mkt_cost_fabs on so_dyeing_mkt_cost_fabs.id=so_dyeing_mkt_cost_qpricedtls.so_dyeing_mkt_cost_fab_id
			where  so_dyeing_mkt_cost_qpricedtls.deleted_at is null
			group by 
			so_dyeing_mkt_cost_qpricedtls.id
        ) fabricDtls"), "fabricDtls.id", "=", "so_dyeing_mkt_cost_qpricedtls.id")
		->when(request('buyer_id'), function ($q) {
			return $q->where('styles.buyer_id', '=', request('buyer_id', 0));
		})
		->when(request('qprice_no'), function ($q) {
			return $q->where('so_dyeing_mkt_cost_qprices.qprice_no', '=', request('qprice_no', 0));
		})
		->when(request('submission_from'), function ($q) {
			return $q->where('so_dyeing_mkt_cost_qprices.qprice_date', '>=',request('submission_from', 0));
		})
		->when(request('submission_to'), function ($q) {
			return $q->where('so_dyeing_mkt_cost_qprices.qprice_date', '<=',request('submission_to', 0));
		})
		->when(request('costing_from'), function ($q) {
			return $q->where('so_dyeing_mkt_costs.quot_date', '>=',request('costing_from', 0));
		})
		->when(request('costing_to'), function ($q) {
			return $q->where('so_dyeing_mkt_costs.quot_date', '<=',request('costing_to', 0));
		})
		->when($approval_type_id, function ($q) use ($approval_type_id){
			if($approval_type_id==1){
				return $q->whereNull('so_dyeing_mkt_cost_qprices.first_approved_at');
			}
			if($approval_type_id==2){
				return $q->whereNotNull('so_dyeing_mkt_cost_qprices.first_approved_at')->whereNull('so_dyeing_mkt_cost_qprices.second_approved_at');
			}
			if($approval_type_id==3){
				return $q->whereNotNull('so_dyeing_mkt_cost_qprices.second_approved_at')->whereNull('so_dyeing_mkt_cost_qprices.third_approved_at');
			}
			if($approval_type_id==10){
				return $q->whereNotNull('so_dyeing_mkt_cost_qprices.third_approved_at')
				->whereNull('so_dyeing_mkt_cost_qprices.final_approved_at');
			}
		})
		->where([['so_dyeing_mkt_cost_qprices.ready_to_approve_id','=',1]])
		->groupBy([
			'so_dyeing_mkt_cost_qprices.id',
			'so_dyeing_mkt_cost_qprices.qprice_no',
			'so_dyeing_mkt_cost_qprices.qprice_date',
    ...
    public function approvalReturn(Request $request){
		...
		$this->approvalcommenthistory->create([
        	'model_id'=>$id,
        	'model_type'=>'so_dyeing_mkt_cost_qprices',
