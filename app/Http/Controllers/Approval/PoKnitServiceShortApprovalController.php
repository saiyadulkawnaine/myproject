...
class PoKnitServiceShortApprovalController extends Controller
{
 ...
 public function __construct(
  ...
  // $this->middleware('permission:approve.poknitserviceshorts',   ['only' => ['approved', 'index', 'reportData', 'reportDataApp', 'unapproved']]);
 }
 public function index()
 {
  $company = array_prepend(array_pluck($this->company->get(), 'name', 'id'), '-Select-', '');
  $supplier = array_prepend(array_pluck($this->supplier->get(), 'name', 'id'), '-Select-', '');
  return Template::loadView('Approval.PoKnitServiceShortApproval', ['company' => $company, 'supplier' => $supplier]);
 }
 public function reportData()
 {
  ...
  $rows = $this->poknitservice
   ...
   ->when(request('supplier_id'), function ($q) {
    return $q->where('po_knit_services.supplier_id', '=', request('supplier_id', 0));
   })
   ...
   ->whereNull('po_knit_services.approved_at')
   ->whereDate('po_knit_services.po_date', '>', '2021-12-31')
   ->where([['po_knit_services.po_type_id', '=', 2]])
 ...
 {
  $id = request('id', 0);
  $master = $this->poknitservice->find($id);
  $user = \Auth::user();
 ...
 public function getKnitServiceSummery()
 {
  $id = request('id', 0);
  ...
  $details = collect(
   \DB::Select("
          ...
          po_knit_service_items.qty as po_qty,
          po_knit_service_items.rate as po_rate,
          po_knit_service_items.amount as po_amount,
          cumulative.qty as total_po_qty,
          cumulative.rate as total_po_rate,
          cumulative.amount as total_po_amount
          from po_knit_services 
          ...
          left join (
              select 
              po_knit_service_items.budget_fabric_prod_id,
              sum(po_knit_service_items.qty) as qty,
              avg(po_knit_service_items.rate) as rate,
              sum(po_knit_service_items.amount) as amount
              from po_knit_service_items 
              join budget_fabric_prods on budget_fabric_prods.id=po_knit_service_items.budget_fabric_prod_id
              group by 
              po_knit_service_items.budget_fabric_prod_id
          ...
          where (po_knit_services.id =  ? ) --27466
          and po_knit_services.deleted_at is null
        ", [$id])
  )
  ...
  $responsible = $this->poknitservice
   ->join('po_knit_service_items', function ($join) {
    $join->on('po_knit_services.id', '=', 'po_knit_service_items.po_knit_service_id');
   })
   ->join('po_knit_service_item_resps', function ($join) {
    $join->on('po_knit_service_items.id', '=', 'po_knit_service_item_resps.po_knit_service_item_id');
   })
   ->join('sections', function ($join) {
    $join->on('sections.id', '=', 'po_knit_service_item_resps.section_id');
   })
   ->leftJoin('employee_h_rs', function ($join) {
    $join->on('employee_h_rs.id', '=', 'po_knit_service_item_resps.employee_h_r_id');
   })
   ->leftJoin('companies', function ($join) {
    $join->on('companies.id', '=', 'employee_h_rs.company_id');
   })
   ->where([['po_knit_services.id', '=', $id]])
   ->orderBy('po_knit_service_item_resps.id', 'desc')
   ->get([
    'po_knit_service_item_resps.*',
  ...
  return Template::loadView('Approval.PoKnitServiceApprovalSummeryMatrix', ['data' => $data]);
 }
 ...
 public function poDetails()
 {
  ...
  $rows = $this->poknitservice
   ->selectRaw('
            ...
            po_knit_services.source_id,
            po_knit_services.pay_mode,
            ...
            avg(po_knit_service_items.rate) as po_rate,
            sum(po_knit_service_items.amount) as po_amount
        ')
   ->join('companies', function ($join) {
    $join->on('companies.id', '=', 'po_knit_services.company_id');
   })
   ...
   ->where([['po_knit_service_items.budget_fabric_prod_id', '=', request('budget_fabric_prod_id', 0)]])
