...
use App\Library\Sms;
class BudgetYarndyeApprovalController extends Controller
{
    private $budget;
    ...
    public function __construct(
        ...
        $this->middleware('permission:approvefirst.budgetyarndye',   ['only' => ['firstapproved']]);
        $this->middleware('permission:approvesecond.budgetyarndye', ['only' => ['secondapproved']]);
        $this->middleware('permission:approvethird.budgetyarndye',   ['only' => ['thirdapproved']]);
        $this->middleware('permission:approvefinal.budgetyarndye', ['only' => ['finalapproved']]);
        $this->middleware('permission:approvefirst.budgetyarndye|approvesecond.budgetyarndye|approvethird.budgetyarndye|approvefinal.budgetyarndye', ['only' => ['approvalReturn']]);
	...
	public function reportData() {
        ...
        ->when($approval_type_id, function ($q) use ($approval_type_id){
            if($approval_type_id==1){
                return $q->whereNull('budget_approvals.yarndye_first_approved_at');
            }
            if($approval_type_id==2){
            return $q->whereNotNull('budget_approvals.yarndye_first_approved_at')->whereNull('budget_approvals.yarndye_second_approved_at');
            }
            if($approval_type_id==3){
            return $q->whereNotNull('budget_approvals.yarndye_second_approved_at')->whereNull('budget_approvals.yarndye_third_approved_at');
            }
            if($approval_type_id==10){
            return $q->whereNotNull('budget_approvals.yarndye_third_approved_at')->whereNull('budget_approvals.yarndye_final_approved_at');
            }
        })
    ...
    public function firstapproved (Request $request)
    {
        ...
        foreach($request->id as $index=>$id){
            if($id){
                ...
                $user = \Auth::user();
                $yarndye_first_approved_at=date('Y-m-d h:i:s');
                $master->yarndye_first_approved_by=$user->id;
                $master->yarndye_first_approved_at=$yarndye_first_approved_at;
    ...
    public function secondapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $yarndye_second_approved_at=date('Y-m-d h:i:s');
                $master->yarndye_second_approved_by=$user->id;
                $master->yarndye_second_approved_at=$yarndye_second_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
    ...
    public function thirdapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $yarndye_third_approved_at=date('Y-m-d h:i:s');
                $master->yarndye_third_approved_by=$user->id;
                $master->yarndye_third_approved_at=$yarndye_third_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
    ...
    public function finalapproved (Request $request)
    {
        foreach($request->id as $index=>$id){
            if($id){
                $master=$this->budgetapproval->where([['budget_id','=',$id]])->get()->first();
                $user = \Auth::user();
                $yarndye_final_approved_at=date('Y-m-d h:i:s');
                $master->yarndye_final_approved_by=$user->id;
                $master->yarndye_final_approved_at=$yarndye_final_approved_at;
                $master->timestamps=false;
                $budget=$master->save();
            }
        }
    ...
    public function approvalReturn(Request $request){
        ...
        $this->approvalcommenthistory->create([
                'model_id'=>$id,
                'model_type'=>'budget_yarn_dyeings',
