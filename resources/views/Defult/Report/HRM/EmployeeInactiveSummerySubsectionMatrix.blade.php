...
<table border="1" cellpadding="3" cellspacing="2" style="margin: 0 auto">
  ...
  <tr>
    ...
    ?>
    <td width="70" class="text-center"><a href="javascript:void(0)" onClick="MsEmployeeInactiveSummery.subSectionEmployeeDetailWindow({{$subsection_id}})">{{ $no_emp }}</a></td>
    <?php
      $subsectionwiseTotal+=$no_emp;
      $monthwiseTotalArr[$key]=isset($monthwiseTotalArr[$key])?$monthwiseTotalArr[$key]+=$no_emp:$no_emp;
    ?>
    @endforeach
    <td width="70" class="text-center"><a href="javascript:void(0)" onClick="MsEmployeeInactiveSummery.subSectionEmployeeDetailWindow({{$subsection_id}})"><strong>{{ $subsectionwiseTotal }}</strong></a></td>
  </tr>
