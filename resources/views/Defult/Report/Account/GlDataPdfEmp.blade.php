<table>
<tr>
<td colspan="10" align="center"><strong>Employee Ledger ( {{ date("d-M-Y",strtotime($start_date))}} To {{ date("d-M-Y",strtotime($end_date))}} )</strong></td>
</tr>
</table>
<br/>
<table  border="1">
        ...
        <th align="center" width="70" class="text-center">Employee</th>
...
@foreach($supvalue as $key=>$value)
        ...
        <td width="70" align="center">{{ $row->employee_name}}</td>
...
@elseif (count($data) <= 0)
    ...
    <tr><td width="638"><strong>{{ $employee[$supkey] }}</strong></td></tr>
    @foreach($supvalue as $key=>$value)
    <?php
         $code = substr($key, 0, 6);
         //print_r($value)
