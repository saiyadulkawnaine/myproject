<table  border="1" style="margin: 0 auto;">
    <caption style="text-align: center;"><img src="{{url('/').'/images/logo/'.$company->logo}}" width="300" height="70" /><br/>{{$company->address}} <br/><font style="font-size: 12px; font-weight: bold;">General Ledger - Opening ({{ $start_date}} To {{ $end_date}})</font></caption>
    <thead>
    <tr>
    ...
    @foreach($data as $key=>$value)
        ...
        @foreach($value as $row)
            ...
            //if($row->trans_type_id==0){
                ...
                <td width="50" align="center"><a href="javascript:void(0)" onClick="MsGlOpn.showJournal({{ $row->id}})">{{ $row->trans_no}}</a></td>
