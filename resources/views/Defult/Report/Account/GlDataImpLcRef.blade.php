...
<table  border="1" style="margin: 0 auto;">
    <caption style="text-align: center;"><img src="{{url('/').'/images/logo/'.$company->logo}}" width="300" height="70" /><br/>{{$company->address}} <br/><font style="font-size: 12px; font-weight: bold;">Import LC Reference Ledger ( {{ date("d-M-Y",strtotime($start_date))}} To {{ date("d-M-Y",strtotime($end_date))}} )</font></caption>
       <thead>
       <tr>
       ...
       <th width="80" class="text-center">Import Lc Reference</th>
...
@if(count($data)>0)
       ...
       <tr><td colspan="10"><strong>{{ $supkey }}</strong></td></tr>
@foreach($supvalue as $key=>$value)         <?php
    ...
    <tr>
       ...
       <td width="50" align="center"><a href="javascript:void(0)" onClick="MsGlImpLcRef.showJournal({{ $row->id}})">{{ $row->trans_no}}</a></td>
       <td width="90" align="center">{{ $row->trans_date}}</td>
       <td width="50" align="center"></td>
       <td width="80" align="center"></td>
       <td width="80" align="center">{{ $row->import_lc_ref_name}}</td>
