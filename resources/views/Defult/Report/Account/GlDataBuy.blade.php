...
<table  border="1" style="margin: 0 auto;">
     <caption style="text-align: center;"><img src="{{url('/').'/images/logo/'.$company->logo}}" width="300" height="70" /><br/>{{$company->address}} <br/><font style="font-size: 12px; font-weight: bold;">Buyer Ledger ( {{ date("d-M-Y",strtotime($start_date))}} To {{ date("d-M-Y",strtotime($end_date))}} )</font></caption>
        <thead>
        <tr>
        ...
        <th width="80" class="text-center">Buyer</th>
        ...
        <tr><td colspan="10"><strong>{{ $buyer[$supkey] }}</strong></td></tr>
@foreach($supvalue as $key=>$value)
        ...
        <td width="50" align="center"><a href="javascript:void(0)" onClick="MsGlBuy.showJournal({{ $row->id}})">{{ $row->trans_no}}</a></td>
        <td width="90" align="center">{{ $row->trans_date}}</td>
        <td width="50" align="center">{{ $row->bill_no}}</td>
        <td width="80" align="center"></td>
        <td width="80" align="center">{{ $row->buyer_name}}</td>
