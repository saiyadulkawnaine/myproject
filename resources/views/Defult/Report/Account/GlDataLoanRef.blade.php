...
<table  border="1" style="margin: 0 auto;">
     <caption style="text-align: center;"><img src="{{url('/').'/images/logo/'.$company->logo}}" width="300" height="70" /><br/>{{$company->address}} <br/><font style="font-size: 12px; font-weight: bold;">Loan Reference Ledger ( {{ date("d-M-Y",strtotime($start_date))}} To {{ date("d-M-Y",strtotime($end_date))}} )</font></caption>
        <thead>
        <tr>
        ...
        <th width="80" class="text-center">Loan Reference</th>
...
@foreach($supvalue as $key=>$value)         <?php
        ...
        <td width="50" align="center"><a href="javascript:void(0)" onClick="MsGlLoanRef.showJournal({{ $row->id}})">{{ $row->trans_no}}</a></td>
        <td width="90" align="center">{{ $row->trans_date}}</td>
        <td width="50" align="center"></td>
        <td width="80" align="center"></td>
        <td width="80" align="center">{{ $row->loan_ref_name}}</td>
