    ...
    <tbody>    
        ...
        @foreach($prodknitqc as $row)
         <tr align="center">
            <td>
            {{ $row->id }}
            <input type="hidden" name="inv_grey_fab_isu_item_id[{{ $i }}]" id="inv_grey_fab_isu_item_id{{ $i }}" value="{{ $row->id }}" readonly />
            <input type="hidden" name="prod_knit_dlv_roll_id[{{ $i }}]" id="prod_knit_dlv_roll_id{{ $i }}" value="{{ $row->prod_knit_dlv_roll_id }}" readonly />
            
            <input type="hidden" name="inv_grey_fab_item_id[{{ $i }}]" id="inv_grey_fab_item_id{{ $i }}" value="{{ $row->inv_grey_fab_item_id }}" readonly />
            ...
            <td>
            <input type="text" name="qty[{{ $i }}]" id="qty{{ $i }}" value="{{ $row->qc_pass_qty }}" readonly />
            </td>

            <td align="left">
            <input type="text" name="store_id[{{ $i }}]" id="store_id{{ $i }}"/>
            {!! Form::select("store_id[$i]",$store,$row->store_id,array('id'=>'store_id',"onchange"=>"MsInvGreyFabTransInItem.copyStore(this.value,$i,$loop->count)")) !!}
            </td>

            <td align="left">
            <input type="text" name="room[{{ $i }}]" id="room{{ $i }}" onchange="MsInvGreyFabTransInItem.copyRoom(this.value,{{ $i }},{{ $loop->count}})"/>
            </td>

            <td align="left">
            <input type="text" name="rack[{{ $i }}]" id="rack{{ $i }}" onchange="MsInvGreyFabTransInItem.copyRack(this.value,{{ $i }},{{ $loop->count}})"/>
            </td>
            <td>
            <input type="text" name="shelf[{{ $i }}]" id="shelf{{ $i }}" onchange="MsInvGreyFabTransInItem.copyShelf(this.value,{{ $i }},{{ $loop->count}})"/>            
            </td>
            <td>
            {{ $row->body_part }}
