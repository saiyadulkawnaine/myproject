...
<input type="hidden" name="budget_yarn_id[{{ $i }}]" id="budget_yarn_id{{ $i }}" class="number integer" value="{{ $colorsize->id}}" readonly/>
</td>
<td width="70px">
<input type="text" name="qty[{{ $i }}]" id="qty{{ $i }}" class="number integer" value="{{ $colorsize->balance_qty}}" onchange="MsPurYarn.calculateAmount({{ $i }},{{$loop->count}},'qty')"/>
</td>
<td>
<input type="text" name="rate[{{ $i  }}]" id="rate_{{ $i }}" class="number integer"  value="{{ $colorsize->rate}}" onchange="MsPurYarn.calculateAmount({{ $i }},{{$loop->count}},'rate')"/>
</td>
<td>
