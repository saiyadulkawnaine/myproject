...
<p>{{ $rows->pay_term }} of this first bill of exchange (second of the same tenor and date being unpaid) to the order of {{ $rows->bank_name }}, {{ $rows->branch_name }}, {{ $rows->bank_address }}</p>
<h2 align="center">1</h2>
<p>The sum of ({{ $rows->inword }})</p>
...
<br pagebreak="true"/>
<p></p>
<p align="center"><img src="{{$image_file}}" width="260", height="100"/></p>
<p align="center" style="padding-top:0;"><span>{{ $rows['company_address'] }}</span></p>
<p align="right">Date: {{ date('d.m.Y') }}</p>
<h3 align="center">BILL OF EXCHANGE</h3>
<h4 align="center">Exchange for {{ $rows->currency_code }} {{ $rows->currency_symbol }}{{ number_format($rows->total_invoice_net_inv_value,2) }}</h4>
<p>{{ $rows->pay_term }} of this second bill of exchange (first of the same tenor and date being unpaid) to the order of {{ $rows->bank_name }}, {{ $rows->branch_name }}, {{ $rows->bank_address }}</p>
