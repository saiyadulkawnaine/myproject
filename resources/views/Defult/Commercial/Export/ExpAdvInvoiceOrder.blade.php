    ...
    <tbody>
        ...
        @foreach($impdocaccepts as $impdocaccept)
                    ...
                    {{ $impdocaccept->sale_order_no }}
                     <input type="hidden" name="exp_adv_invoice_id[{{ $i }}]" id="exp_adv_invoice_id{{ $i }}" value="{{ $impdocaccept->exp_adv_invoice_id }}"/>
                    <input type="hidden" name="exp_pi_order_id[{{ $i }}]" id="exp_pi_order_id{{ $i }}" value="{{ $impdocaccept->exp_pi_order_id }}"/>
                    </td>
                    ...
                    <td>
                    <input type="text" name="qty[{{ $i }}]" id="qty{{ $i }}" class="number integer" value=" {{ $impdocaccept->invoice_qty}}" onchange="MsExpAdvInvoiceOrder.calculate({{ $i }})"/>
                    </td>

                    <td>
                    <input type="text" name="rate[{{ $i }}]" id="rate{{ $i }}" class="number integer" value=" {{ $impdocaccept->rate}}" onchange="MsExpAdvInvoiceOrder.calculate({{ $i }})" />
                    </td>

                    <td>
                    <input type="text" name="amount[{{ $i }}]" id="amount{{ $i }}" class="number integer" value=" {{ $impdocaccept->amount}}" readonly />
                    </td>
    ...
    <tbody>
        ...
        @foreach($saved as $row)
                  ...
                  ?>
                    ...
                    <td width="100px">
                    {{ $row->sale_order_no }}
                     <input type="hidden" name="exp_adv_invoice_id[{{ $i }}]" id="exp_adv_invoice_id{{ $i }}" value="{{ $row->exp_adv_invoice_id }}"/>
                    <input type="hidden" name="exp_pi_order_id[{{ $i }}]" id="exp_pi_order_id{{ $i }}" value="{{ $row->exp_pi_order_id }}"/>
                    </td>
                    ...
                    <td>
                    <input type="text" name="qty[{{ $i }}]" id="qty{{ $i }}" class="number integer" value=" {{ $row->invoice_qty }}" onchange="MsExpAdvInvoiceOrder.calculate({{$i}})"/>
                    </td>

                    <td>
                    <input type="text" name="rate[{{ $i }}]" id="rate{{ $i }}" class="number integer" value="{{ $row->invoice_rate }}" onchange="MsExpAdvInvoiceOrder.calculate({{$i}})" />
                    </td>

                    <td>
                        <a href="javascript:void(0)" onClick="MsExpAdvInvoiceOrderDtl.createOrderDtl({{$row->exp_adv_invoice_order_id}},{{$row->invoice_qty}})">
                    ...
                    {{ $row->marchent}}
                     ...
                     <td>
                    {!! Form::select("production_source_id[$i]", $productionsource,$row->production_source_id,array('id'=>'production_source_id')) !!}
                    </td>
                    ...
                    <td>
                    	<a href="javascript:void(0)" onclick="MsExpAdvInvoiceOrder.delete(event,{{ $row->exp_adv_invoice_order_id }})">Remove</a>
                    </td>
            </tr>
           <?php
      $i++;
