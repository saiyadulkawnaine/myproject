...
<table border="1" cellspacing="0" cellpadding="0">
    <thead>
        <tr>
            <td rowspan="2" align="center" width="163"><strong>MARKS & NUMBERS OF PKGS</strong></td>
            <td colspan="6" align="center"  width="510"><strong>DESCRIPTION OF GOODS</strong></td>
        </tr>
        <tr>
            <td align="center" width="100"><strong>STYLE NO/<br/>ARTICLE NO</strong></td>
            <td align="center" width="100"><strong>PO NO/<br/>ORDER NO</strong></td>
            <td align="center" width="110"><strong>GARMENTS ITEM & FABRICATION</strong></td>
            <td align="center" width="70"><strong>QUANTITY IN<br/> {{ strtoupper($rows->uom_code) }}</strong></td>
            <td align="center" width="60"><strong>PRICE PER<br/> {{ strtoupper($rows->uom_code) }}</strong></td>
            <td align="center" width="70"><strong>AMOUNT<br/> IN {{ $rows->currency_code }}&nbsp;</strong></td>
        </tr>
    </thead>
    <?php
        $total_qty_pcs=0;
    ...
    <tbody>
        <tr>
            <td width="163">{!! nl2br(e($rows->shipping_mark)) !!}</td>
            <td colspan="6" style="margin-left: 0px"  width="510">@foreach($expinvoiceorder as $item)<table border="1" cellspacing="0" cellpadding="2" style="margin-left: 0px">
                    <tr>
                        <td style="margin-left: 0px"  width="100">{{ $item->style_ref }}<br/>{{ $item->article_no }}</td>
                        <td style="margin-left: 0px"  width="100">{{ $item->sale_order_no }}</td>
                        <td style="margin-left: 0px"  width="110">{{ $item->commodity }}</td>
                        <td style="margin-left: 0px" align="right"  width="70">{{ number_format($item->invoice_qty,0) }}</td>
                        <td style="margin-left: 0px" align="right"  width="60">{{ number_format($item->invoice_rate,4) }}</td>
                        <td style="margin-left: 0px" align="right" width="70">{{ number_format($item->invoice_amount,2) }}</td>
                    </tr>
                </table>
                <?php
                    $total_qty_pcs+=$item->qty;
                    $total_invoice_qty+=$item->invoice_qty;
    ...
    <tfoot>
        ...
        ?>
        <tr>
            <td align="right" colspan="4" width="473"><strong>Gross Value</strong>&nbsp;&nbsp;</td>
            <td align="right" width="70" >{{ number_format($total_invoice_qty,0) }}&nbsp;&nbsp;&nbsp;</td>
            <td align="left" width="60" >&nbsp;&nbsp;<strong>{{ strtoupper($rows->uom_code) }}</strong></td>
            <td align="right" width="70">{{ $rows->currency_symbol }}&nbsp;{{ number_format($totalInvoiceAmount,2) }}</td>
        </tr>
        @if ($rows->discount_amount)
        <tr>
            <td colspan="6" width="603">{{ $rows->discount_detail }}</td>
            <td align="right" width="70">{{ number_format($rows->discount_amount,2) }}</td>  
        </tr> 
        @endif
       @if ($rows->bonus_amount)
        <tr>
            <td colspan="6" width="603">{{ $rows->bonus_detail }}</td>
            <td align="right" width="70">{{ number_format($rows->bonus_amount,2) }}</td>  
        </tr> 
       @endif
        @if ($rows->claim_amount)
        <tr>
            <td colspan="6" width="603">{{ $rows->claim_detail }}</td>
            <td align="right" width="70">{{ number_format($rows->claim_amount,2) }}</td>  
        </tr>   
        @endif
        @if ($rows->commission)
        <tr>
            <td colspan="6" width="603">{{ $rows->commision_remarks }}</td>
            <td align="right" width="70">{{ number_format($rows->commission,2) }}</td>  
        </tr>  
        @endif
        @if ($rows->up_charge_amount)
        <tr>
            <td colspan="6" width="603">{{ $rows->up_charge_remarks }}</td>
            <td align="right" width="70">{{ number_format($rows->up_charge_amount,2) }}</td>  
        </tr>   
        @endif
        <tr>
            <td colspan="6" align="right" width="603"><strong>Net Value</strong>&nbsp;&nbsp;</td>
            <td align="right" width="70">{{ $rows->currency_symbol }}&nbsp;{{ number_format($netInvoiceValue,2) }}</td>  
        </tr>
    </tfoot>
</table>
