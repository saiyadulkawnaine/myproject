    ...
    <tbody>
        ...
        <tr align="center">
            ...
            <td width="200px">
                ...
                <input type="hidden" name="exp_adv_invoice_order_id[{{ $i }}]" id="exp_adv_invoice_id{{ $i }}" value="{{ $colorsize->exp_adv_invoice_order_id }}"/>
            ...
            <td width="80px" align="right">
                <input type="text" name="qty[{{ $i }}]" id="qty{{ $i }}" class="number integer" onchange="MsExpAdvInvoiceOrderDtl.calculate({{ $i }},{{ $loop->count}})" value=""/>
            </td>
            <td width="80px" align="right">
                <input type="text" name="rate[{{ $i }}]" id="rate{{ $i }}" class="number integer" onchange="MsExpAdvInvoiceOrderDtl.calculate({{ $i }},{{ $loop->count}})" value=" {{ $colorsize->color_rate }} "/>
            </td>
            <td width="80px">
...
<table border="1">
    ...
    <tbody>
        ...
        <tr align="center">
            ...
            <td width="80px" align="right">
                <input type="text" name="qty[{{ $i }}]" id="qty{{ $i }}" class="number integer" onchange="MsExpAdvInvoiceOrderDtl.calculate({{ $i }},{{ $loop->count}})" value="{{ $colorsize->qty }}"/>
            </td>
            <td width="80px" align="right">
                <input type="text" name="rate[{{ $i }}]" id="rate{{ $i }}" class="number integer" onchange="MsExpAdvInvoiceOrderDtl.calculate({{ $i }},{{ $loop->count}})" value="{{ $colorsize->rate }}"/>
            </td>
            <td width="80px">
            <input type="text" name="amount[{{ $i }}]" id="amount_{{ $i }}" class="number" value="{{ $colorsize->amount }}" readonly/>
            </td>
            <td width="80px"><a href="javascript:void(0)" onclick="MsExpAdvInvoiceOrderDtl.delete(event,{{ $colorsize->exp_adv_invoice_order_dtl_id }})">Remove</a></td>
        </tr>
