...
<table><tr><td>
<ol>
    <li>SALES CONTRACT NO & DATE : {{$explcsc['lc_sc_no']}}</li>
    <li>NAME AND ADDRESS OF THE APPLICANT : {{$explcsc['buyer']}} , {{ $buyerbranch['address'] }}</li>
    <li>APPLICANT'S BANK : {{$explcsc['buyers_bank']}}</li>
    <li><strong>CONSIGNEE / NOTIFYING PARTY : </strong><br>
        <table border="0">
            <tr>
                <td><p> {{$explcsc['consignee_id']}} ,<br> {{ $buyerbranch['address'] }}</p></td>
                <td><p> {{$explcsc['notifying_party_id']}} ,<br> {{ $buyerbranch['address'] }}</p></td>
            </tr>
        </table>
    </li>
    <li>SUPPLIER / SHIPPER : {{ $explcsc['beneficiary_id'] }}, {{ $company['address'] }}</li>
    <li>NAME AND ADDRESS OF SHIPPER BANK : {{ $explcsc['exporter_bank_branch_id'] }} , {{ $bankbranch['address'] }}</li>
