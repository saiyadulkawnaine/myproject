         ...
         <?php
			...
			?>
                    ...
                    {{ $colorsize->item_description }}
                     <input type="hidden" name="smp_cost_id[{{ $i }}]" id="smp_cost_id_{{ $i }}" value="{{ $colorsize->smp_cost_id }}"/>
                    ...
                    <td width="70px">
                    <input type="hidden" name="cons[{{ $i }}]" id="cons_{{ $i }}" class="number integer" onchange="MsSmpCostEmb.calculate({{ $i }},{{ $loop->count}},'cons')" value="{{ $cons }}"/>

                    <a href="javascript:void(0)" onclick="MsSmpCostEmb.openConsWindow({{ $colorsize->id }})">{{ $cons }}</a>
                    </td>
                    <td>
                    <input style="width:95%" type="text" name="rate[{{ $i  }}]" id="rate_{{ $i }}" class="number integer" onchange="MsSmpCostEmb.calculate({{ $i }},{{$loop->count}},'rate')" value="{{ $colorsize->rate }}" readonly />
                    </td>
