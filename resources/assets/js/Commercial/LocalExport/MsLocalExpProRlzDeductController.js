let MsLocalExpProRlzDeductModel = require('./MsLocalExpProRlzDeductModel');
class MsLocalExpProRlzDeductController {
	constructor(MsLocalExpProRlzDeductModel)
	{
		this.MsLocalExpProRlzDeductModel = MsLocalExpProRlzDeductModel;
		this.formId='localexpprorlzdeductFrm';
		this.dataTable='#localexpprorlzdeductTbl';
		this.route=msApp.baseUrl()+"/localexpprorlzdeduct"
	}
	...
	submit()
	{
		...
		if(formObj.id){
			this.MsLocalExpProRlzDeductModel.save(this.route+"/"+formObj.id,'PUT',msApp.qs.stringify(formObj),this.response);
		}else{
			this.MsLocalExpProRlzDeductModel.save(this.route,'POST',msApp.qs.stringify(formObj),this.response);
		}
	}	
	...
	{
		let formObj=msApp.get(this.formId);
		this.MsLocalExpProRlzDeductModel.save(this.route+"/"+formObj.id,'DELETE',null,this.response);
	}

	delete(event,id)
	{
		event.stopPropagation()
		this.MsLocalExpProRlzDeductModel.save(this.route+"/"+id,'DELETE',null,this.response);
	}

	response(d)
	{
		$('#localexpprorlzdeductTbl').datagrid('reload');
		msApp.resetForm('localexpprorlzdeductFrm');
		$('#localexpprorlzdeductFrm [name=local_exp_pro_rlz_id]').val($('#localexpprorlzFrm [name=id]').val());
	}
	...
	edit(index,row)
	{
		...
		this.MsLocalExpProRlzDeductModel.get(index,row);
	...
	showGrid(data)
	{
		$('#frdeTbl').datagrid({
			...
			iconCls:'icon-edit',
			width:660,
			toolbar:"#localexpprorlzTb",
			height:250,
			fit:true,
			showFooter:true,
			singleSelect:true,
			idField:'id',
			//data:data.localexpprorlzdeduct,
			columns:[[
				{
			...
			onLoadSuccess: function(data){
				...
				MsLocalExpProRlzDeduct.reloadFooter(tdata);
			...
			onBeginEdit:function(rowIndex){
				...
				n1.add(n2).add(n3).numberbox({
					onChange:function()
					{
						...
						var data={};
						data=MsLocalExpProRlzDeduct.gettotal(rowIndex);
						MsLocalExpProRlzDeduct.reloadFooter(data);
		...
		}).datagrid('loadData',data.localexpprorlzdeduct);
	}

	formatDetail(value,row)
	{
		return '<a href="javascript:void(0)"  onClick="MsLocalExpProRlzDeduct.delete(event,'+row.id+')"><span class="btn btn-danger btn-xs"><i class="fa fa-search"></i>Delete</span></a>';
	}
	 ...
	 reloadFooter(data)
      ...
      dom_value: data.total_dom_value.toFixed(4).replace(/\d(?=(\d{3})+\.)/g, '$&,')
       ...
       editrow(target){
         $('#frdeTbl').datagrid('beginEdit', MsLocalExpProRlzDeduct.getRowIndex(target));
      }
	...
	insert(){
		var row = $('#frdeTbl').datagrid('getSelected');
         if (row){
            var index = $('#frdeTbl').datagrid('getRowIndex',row);
            if(!MsLocalExpProRlzDeduct.beforeInsert(index)){
               alert('insert value')
               return ;
            }
		} else {
		...
		var data={};
		data=MsLocalExpProRlzDeduct.gettotal(index);
		MsLocalExpProRlzDeduct.reloadFooter(data);
...
window.MsLocalExpProRlzDeduct=new MsLocalExpProRlzDeductController(new MsLocalExpProRlzDeductModel());
