...
let MsOrderWiseBudgetModel = require('./MsOrderWiseBudgetModel');
require('./datagrid-filter.js');

class MsOrderWiseBudgetController {
	constructor(MsOrderWiseBudgetModel)
	{
		this.MsOrderWiseBudgetModel = MsOrderWiseBudgetModel;
		this.formId='orderwisebudgetFrm';
		this.dataTable='#orderwisebudgetTbl';
		this.route=msApp.baseUrl()+"/orderwisebudget/getdata"
	}
	
	get(){
		let params={};
		params.company_id = $('#orderwisebudgetFrm  [name=company_id]').val();
		params.buyer_id = $('#orderwisebudgetFrm  [name=buyer_id]').val();
		params.style_ref = $('#orderwisebudgetFrm  [name=style_ref]').val();
		params.job_no = $('#orderwisebudgetFrm  [name=job_no]').val();
		params.date_from = $('#orderwisebudgetFrm  [name=date_from]').val();
		params.date_to = $('#orderwisebudgetFrm  [name=date_to]').val();
		params.order_status = $('#orderwisebudgetFrm  [name=order_status]').val();
		
		let d= axios.get(this.route,{params})
		.then(function (response) {
			//MsProjectionProgress.showGrid(response.data.datad)
			$('#orderwisebudgetTbl').datagrid('loadData', response.data);
		})
	...
	{
        return '<img href="javascript:void(0)" width="15"  height="15" src="'+msApp.baseUrl()+'/images/'+row.flie_src+'" onClick="MsOrderWiseBudget.imageWindow('+'\''+row.flie_src+'\''+')"/>';
	}

	formatSaleOrder(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsSaleOrderWindow('+row.id+')">'+row.qty+'</a>';
	}

	formatYarn(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsYarnWindow('+row.id+')">'+row.yarn_amount+'</a>';
	}
	formatTrim(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsTrimWindow('+row.id+')">'+row.trim_amount+'</a>';
	}
	formatKnit(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsKnitWindow('+row.id+')">'+row.kniting_amount+'</a>';
	}
	formatYarnDying(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsYarnDyeingWindow('+row.id+')">'+row.yarn_dying_amount+'</a>';
	}
	formatWeaving(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsWindow('+row.id+')">'+row.weaving_amount+'</a>';
	}
	formatDyeing(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsDyeingWindow('+row.id+')">'+row.dying_amount+'</a>';
	}
	formatAop(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsAopWindow('+row.id+')">'+row.aop_amount+'</a>';
	}

	formatBurnOut(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsBocWindow('+row.id+')">'+row.burn_out_amount+'</a>';
	}
	formatFinishing(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsWindow('+row.id+')">'+row.finishing_amount+'</a>';
	}
	formatWashing(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsFwcWindow('+row.id+')">'+row.washing_amount+'</a>';
	}

	formatPrinting(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsGpcWindow('+row.id+')">'+row.printing_amount+'</a>';
	}

	formatEmb(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsGecWindow('+row.id+')">'+row.emb_amount+'</a>';
	}

	formatSpEmb(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsGsecWindow('+row.id+')">'+row.spemb_amount+'</a>';
	}

	formatGmtDying(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsGdcWindow('+row.id+')">'+row.gmt_dyeing_amount+'</a>';
	}

	formatGmtWashing(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsGwcWindow('+row.id+')">'+row.gmt_washing_amount+'</a>';
	}

	formatCourier(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsOthWindow('+row.id+')">'+row.courier_amount+'</a>';
	}

	formatFreight(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsWindow('+row.id+')">'+row.freight_amount+'</a>';
	}

	formatCm(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsWindow('+row.id+')">'+row.cm_amount+'</a>';
	}

	formatCommi(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsWindow('+row.id+')">'+row.commi_amount+'</a>';
	}

	formatCommer(value,row)
	{
        return '<a href="javascript:void(0)" onClick="MsOrderWiseBudget.detailsWindow('+row.id+')">'+row.commer_amount+'</a>';
	}
...
window.MsOrderWiseBudget=new MsOrderWiseBudgetController(new MsOrderWiseBudgetModel());
MsOrderWiseBudget.showGrid({rows :{}});
MsOrderWiseBudget.showGridYarn({rows :{}});
MsOrderWiseBudget.showGridYarn2({rows :{}});
MsOrderWiseBudget.showGridTrim({rows :{}});
MsOrderWiseBudget.showGridTrim2({rows :{}});
MsOrderWiseBudget.showGridKnit({rows :{}});
MsOrderWiseBudget.showGridKnit2({rows :{}});
MsOrderWiseBudget.showGridYarnDyeing({rows :{}});
MsOrderWiseBudget.showGridYarnDyeing2({rows :{}});
MsOrderWiseBudget.showGridDyeing({rows :{}});
MsOrderWiseBudget.showGridDyeing2({rows :{}});
MsOrderWiseBudget.showGridAop({rows :{}});
MsOrderWiseBudget.showGridAop2({rows :{}});

MsOrderWiseBudget.showGridBoc({rows :{}});
MsOrderWiseBudget.showGridBoc2({rows :{}});

MsOrderWiseBudget.showGridFwc({rows :{}});
MsOrderWiseBudget.showGridFwc2({rows :{}});
MsOrderWiseBudget.showGridGpc({rows :{}});
MsOrderWiseBudget.showGridGpc2({rows :{}});

MsOrderWiseBudget.showGridGec({rows :{}});
MsOrderWiseBudget.showGridGec2({rows :{}});

MsOrderWiseBudget.showGridGsec({rows :{}});
MsOrderWiseBudget.showGridGsec2({rows :{}});

MsOrderWiseBudget.showGridGdc({rows :{}});
MsOrderWiseBudget.showGridGdc2({rows :{}});

MsOrderWiseBudget.showGridGwc({rows :{}});
MsOrderWiseBudget.showGridGwc2({rows :{}});

MsOrderWiseBudget.showGridOth({rows :{}});
MsOrderWiseBudget.showGridOth2({rows :{}});
