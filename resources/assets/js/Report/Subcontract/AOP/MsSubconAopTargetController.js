...
let MsSubconAopTargetModel = require('./MsSubconAopTargetModel');

class MsSubconAopTargetController {
	constructor(MsSubconAopTargetModel)
	{
		this.MsSubconAopTargetModel = MsSubconAopTargetModel;
		this.formId='subconaoptargetFrm';
		this.dataTable='#subconaoptargetTbl';
		this.route=msApp.baseUrl()+"/subconaoptarget";
	}

	getParams(){
		let params={};
		params.date_from = $('#subconaoptargetFrm  [name=date_from]').val();
		params.date_to = $('#subconaoptargetFrm  [name=date_to]').val();
		return params;
	}
	...
	get()
	{
		...
		.then(function (response) {
			$('#subconaoptargetTbl').datagrid('loadData', response.data);
		})
		...
		var title='Subcontract Aop Target Report : &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp From '+formatted_date_from+' &nbsp&nbspTo &nbsp&nbsp'+formatted_date_to;
		var p = $('#subconaoptargetpanel').layout('panel', 'center').panel('setTitle', title);
	...
	formatContact(value,row){
		return '<a href="javascript:void(0)" onClick="MsSubconAopTarget.contactWindow('+row.buyer_id+')">'+value+'</a>';
	}

	contactWindow(buyer_id)
	{
		
		//$('#subconaoptargetcontactTbl').datagrid('loadData',row);
		//$('#subconaoptargetWindow').window('open');
		//let params=this.getParams();
		let params={};
		...
		let sq=data.then(function (response) {
			$('#subconaoptargetWindow').window('open');
			$('#subconaoptargetcontactTbl').datagrid('loadData',response.data);
		})
		.catch(function (error) {
			console.log(error);
		});
	...
	{
		var dg = $('#subconaoptargetcontactTbl');
		dg.datagrid({
			border:false,
			singleSelect:true,
			showFooter:true,
...
window.MsSubconAopTarget=new MsSubconAopTargetController(new MsSubconAopTargetModel());
MsSubconAopTarget.showGrid([]);
MsSubconAopTarget.showGridContract([]);