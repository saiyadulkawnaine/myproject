...
let MsSubconDyeingOrderProgressModel = require('./MsSubconDyeingOrderProgressModel');

class MsSubconDyeingOrderProgressController {
	constructor(MsSubconDyeingOrderProgressModel)
	{
		this.MsSubconDyeingOrderProgressModel = MsSubconDyeingOrderProgressModel;
		this.formId='subcondyeingorderprogressFrm';
		this.dataTable='#subcondyeingorderprogressTbl';
		this.route=msApp.baseUrl()+"/subcondyeingorderprogress";
	}
	
	get(){
		let params={};
		params.company_id = $('#subcondyeingorderprogressFrm  [name=company_id]').val();
		params.buyer_id = $('#subcondyeingorderprogressFrm  [name=buyer_id]').val();
		params.teammember_id = $('#subcondyeingorderprogressFrm  [name=teammember_id]').val();
		params.sales_order_no = $('#subcondyeingorderprogressFrm  [name=sales_order_no]').val();
		params.rcv_date_from = $('#subcondyeingorderprogressFrm  [name=rcv_date_from]').val();
		params.rcv_date_to = $('#subcondyeingorderprogressFrm  [name=rcv_date_to]').val();
		params.dlv_date_from = $('#subcondyeingorderprogressFrm  [name=dlv_date_from]').val();
		params.dlv_date_to = $('#subcondyeingorderprogressFrm  [name=dlv_date_to]').val();

		let d= axios.get(this.route+"/getdata",{params})
		.then(function (response) {
			$('#subcondyeingorderprogressTbl').datagrid('loadData', response.data);
		})
...
window.MsSubconDyeingOrderProgress= new MsSubconDyeingOrderProgressController(new MsSubconDyeingOrderProgressModel());
MsSubconDyeingOrderProgress.showGrid([]);