...
let MsSubconDyeingTargetModel = require('./MsSubconDyeingTargetModel');

class MsSubconDyeingTargetController {
	constructor(MsSubconDyeingTargetModel)
	{
		this.MsSubconDyeingTargetModel = MsSubconDyeingTargetModel;
		this.formId='subcondyeingtargetFrm';
		this.dataTable='#subcondyeingtargetTbl';
		this.route=msApp.baseUrl()+"/subcondyeingtarget";
	}

	getParams(){
		let params={};
		params.date_from = $('#subcondyeingtargetFrm  [name=date_from]').val();
		params.date_to = $('#subcondyeingtargetFrm  [name=date_to]').val();
		return params;
	}
	...
	get()
	{
		...
		.then(function (response) {
			$('#subcondyeingtargetTbl').datagrid('loadData', response.data);
		})
		...
		var title='Subcontract Dyeing Target Report : &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp From '+formatted_date_from+' &nbsp&nbspTo &nbsp&nbsp'+formatted_date_to;
		var p = $('#subcondyeingtargetpanel').layout('panel', 'center').panel('setTitle', title);
	...
	formatContact(value,row){
		return '<a href="javascript:void(0)" onClick="MsSubconDyeingTarget.contactWindow('+row.buyer_id+')">'+value+'</a>';
	}

	contactWindow(buyer_id)
	{
		
		//$('#subcondyeingtargetcontactTbl').datagrid('loadData',row);
		//$('#subcondyeingtargetWindow').window('open');
		//let params=this.getParams();
		let params={};
		...
		let sq=data.then(function (response) {
			$('#subcondyeingtargetWindow').window('open');
			$('#subcondyeingtargetcontactTbl').datagrid('loadData',response.data);
		})
		.catch(function (error) {
			console.log(error);
		});
	...
	{
		var dg = $('#subcondyeingtargetcontactTbl');
		dg.datagrid({
			border:false,
			singleSelect:true,
			showFooter:true,
...
window.MsSubconDyeingTarget=new MsSubconDyeingTargetController(new MsSubconDyeingTargetModel());
MsSubconDyeingTarget.showGrid([]);
MsSubconDyeingTarget.showGridContract([]);