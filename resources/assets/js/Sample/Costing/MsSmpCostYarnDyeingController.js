...
class MsSmpCostYarnDyeingController {
	constructor(MsSmpCostFabricProdModel)
	{
		this.MsSmpCostFabricProdModel = MsSmpCostFabricProdModel;
		this.formId='smpcostyarndyeingFrm';
		this.dataTable='#smpcostyarndyeingTbl';
	...
	response(d)
	{
		...
		//MsSmpCostCommercial.get(smp_cost_id);
		//MsSmpCostProfit.get(smp_cost_id);
		//MsSmpCostCommission.get(smp_cost_id);
		//MsSmpCost.reloadDetails(smp_cost_id);
	}
	...
	edit(index,row)
	{
		...
		MsSmpCostYarnDyeing.setClass(row.production_area_id)
		let data=MsSmpCostYarnDyeing.getyarncount (row.smp_cost_fabric_id)
		.then(function (response) {
			  MsSmpCostYarnDyeing.MsSmpCostFabricProdModel.get(index,row);
		})
	...
	get(smp_cost_id)
	{
		let data= axios.get(this.route+"?smp_cost_id="+smp_cost_id+'&production_area_id=5')
		.then(function (response) {
			MsSmpCostYarnDyeing.showGrid(response.data)
		})
		.catch(function (error) {
			console.log(error);
		});
	...
	calculatemount()
	{
		let cons;
		let rate;
		cons= $('#smpcostyarndyeingFrm  [name=cons]').val();
		rate=$('#smpcostyarndyeingFrm  [name=rate]').val();
		let amount=cons*rate;
		$('#smpcostyarndyeingFrm  [name=amount]').val(amount)
	}
	...
	getFabricCons(smp_cost_fabric_id)
	{
		...
		let g=data.then(function (response) {
			$.each(response.data, function(key, value) {
				$('#smpcostyarndyeingFrm [name="cons"]').val(value.req_cons);
				$('#smpcostyarndyeingFrm [name="req_cons"]').val(value.req_cons);
				MsSmpCostYarnDyeing.calculatemount();
			});
		})
		.catch(function (error) {
			console.log(error);
		});
		MsSmpCostYarnDyeing.getyarncount (smp_cost_fabric_id);
	}
	...
	getRate()
	{
		let smp_cost_fabric_id=$('#smpcostyarndyeingFrm [name="smp_cost_fabric_id"]').val();
		let production_process_id=$('#smpcostyarndyeingFrm [name="production_process_id"]').val();
		let colorrange_id=$('#smpcostyarndyeingFrm [name="colorrange_id"]').val();
		let yarncount_id=$('#smpcostyarndyeingFrm [name="yarncount_id"]').val();
		let cons=$('#smpcostyarndyeingFrm [name="cons"]').val();
		...
		.then(function (response) {
			if(response.data)
			{
			   $('#smpcostyarndyeingFrm [name="rate"]').val(response.data.rate) 
			   $('#smpcostyarndyeingFrm [name="amount"]').val(response.data.rate*cons) 
			}
			else
			{
	...
	processChange()
	{
		$('#smpcostyarndyeingFrm [name="rate"]').val('') 		
		let production_process_id=$('#smpcostyarndyeingFrm [name="production_process_id"]').val();
		let data= axios.get(this.route+"/productionarea?production_process_id="+production_process_id)
		.then(function (response) {
			$('#smpcostyarndyeingFrm [name="production_area_id"]').val(response.data.production_area_id)
			MsSmpCostYarnDyeing.setClass(response.data.production_area_id);
			MsSmpCostYarnDyeing.setReadOnly(response.data.production_area_id);
		})
		.catch(function (error) {
			console.log(error);
		});
	...
	setReadOnly(production_area_id){
		...
		{
			 $('#smpcostyarndyeingFrm [name="rate"]').attr("readonly", true);
		}
		else
		{
			$('#smpcostyarndyeingFrm [name="rate"]').attr("readonly", false);
		}
...
window.MsSmpCostYarnDyeing=new MsSmpCostYarnDyeingController(new MsSmpCostFabricProdModel());
