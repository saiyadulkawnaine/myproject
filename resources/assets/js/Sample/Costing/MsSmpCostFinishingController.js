...
class MsSmpCostFinishingController {
	constructor(MsSmpCostFabricProdModel)
	{
		this.MsSmpCostFabricProdModel = MsSmpCostFabricProdModel;
		this.formId='smpcostfinishingFrm';
		this.dataTable='#smpcostfinishingTbl';
	...
	edit(index,row)
	{
		...
		MsSmpCostFinishing.setClass(row.production_area_id)
		let data=MsSmpCostFinishing.getyarncount (row.smp_cost_fabric_id)
		.then(function (response) {
			  MsSmpCostFinishing.MsSmpCostFabricProdModel.get(index,row);
		})
	...
	get(smp_cost_id)
	{
		let data= axios.get(this.route+"?smp_cost_id="+smp_cost_id+'&production_area_id=30')
		.then(function (response) {
			MsSmpCostFinishing.showGrid(response.data)
		})
		.catch(function (error) {
			console.log(error);
		});
	...
	calculatemount()
	{
		let cons;
		let rate;
		cons= $('#smpcostfinishingFrm  [name=cons]').val();
		rate=$('#smpcostfinishingFrm  [name=rate]').val();
		let amount=cons*rate;
		$('#smpcostfinishingFrm  [name=amount]').val(amount)
	}
	...
	getFabricCons(smp_cost_fabric_id)
	{
		...
		let g=data.then(function (response) {
			$.each(response.data, function(key, value) {
				$('#smpcostfinishingFrm [name="cons"]').val(value.req_cons);
				$('#smpcostfinishingFrm [name="req_cons"]').val(value.req_cons);
				MsSmpCostFinishing.calculatemount();
			});
		})
		.catch(function (error) {
			console.log(error);
		});
		MsSmpCostFinishing.getyarncount (smp_cost_fabric_id);
	}
	...
	getRate()
	{
		let smp_cost_fabric_id=$('#smpcostfinishingFrm [name="smp_cost_fabric_id"]').val();
		let production_process_id=$('#smpcostfinishingFrm [name="production_process_id"]').val();
		let colorrange_id=$('#smpcostfinishingFrm [name="colorrange_id"]').val();
		let yarncount_id=$('#smpcostfinishingFrm [name="yarncount_id"]').val();
		let cons=$('#smpcostfinishingFrm [name="cons"]').val();
		...
		.then(function (response) {
			if(response.data)
			{
			   $('#smpcostfinishingFrm [name="rate"]').val(response.data.rate) 
			   $('#smpcostfinishingFrm [name="amount"]').val(response.data.rate*cons) 
			}
			else
			{
	...
	processChange()
	{
		$('#smpcostfinishingFrm [name="rate"]').val('') 		
		let production_process_id=$('#smpcostfinishingFrm [name="production_process_id"]').val();
		let data= axios.get(this.route+"/productionarea?production_process_id="+production_process_id)
		.then(function (response) {
			$('#smpcostfinishingFrm [name="production_area_id"]').val(response.data.production_area_id)
			MsSmpCostFinishing.setClass(response.data.production_area_id);
			MsSmpCostFinishing.setReadOnly(response.data.production_area_id);
		})
		.catch(function (error) {
			console.log(error);
		});
	...
	setReadOnly(production_area_id){
		...
		{
			 $('#smpcostfinishingFrm [name="rate"]').attr("readonly", true);
		}
		else
		{
			$('#smpcostfinishingFrm [name="rate"]').attr("readonly", false);
		}
...
window.MsSmpCostFinishing=new MsSmpCostFinishingController(new MsSmpCostFabricProdModel());
