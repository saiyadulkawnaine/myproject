...
class CreateSoAopMktCostQpricesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('so_aop_mkt_cost_qprices', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('so_aop_mkt_cost_id');
            $table->foreign('so_aop_mkt_cost_id','aopmktcostQuotePriceID')->references('id')->on('so_aop_mkt_costs')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('so_aop_mkt_cost_qprices');
    }
}
