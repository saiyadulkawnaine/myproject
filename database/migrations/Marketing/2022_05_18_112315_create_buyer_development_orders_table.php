...
class CreateBuyerDevelopmentOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('buyer_development_orders', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('buyer_development_intm_id')->unsigned();
            $table->foreign('buyer_development_intm_id')->references('id')->on('buyer_development_intms')->onDelete('cascade');
